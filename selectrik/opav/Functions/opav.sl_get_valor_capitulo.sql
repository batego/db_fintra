-- Function: opav.sl_get_valor_capitulo(integer, integer)

-- DROP FUNCTION opav.sl_get_valor_capitulo(integer, integer);

CREATE OR REPLACE FUNCTION opav.sl_get_valor_capitulo(id_disc_ integer, id_rel_ integer)
  RETURNS text AS
$BODY$
...
begin
   ...
   select   (select
            ...
            where
            /*a.id_solicitud=ap.id_solicitud and*/ d.id=cd.id and f.reg_status='' and e.reg_status='') as valor into retorno
...
ALTER FUNCTION opav.sl_get_valor_capitulo(integer, integer)
  OWNER TO postgres;
