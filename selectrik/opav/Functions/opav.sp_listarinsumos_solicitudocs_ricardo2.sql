-- Function: opav.sp_listarinsumos_solicitudocs_ricardo2(character varying, character varying, character varying, character varying)

-- DROP FUNCTION opav.sp_listarinsumos_solicitudocs_ricardo2(character varying, character varying, character varying, character varying);

CREATE OR REPLACE FUNCTION opav.sp_listarinsumos_solicitudocs_ricardo2(idsolicitud character varying, _usuario character varying, _accion character varying, _codsol character varying)
		...
		if ( verify = true ) then
			...
			FOR result IN
                                ...
                                SELECT
                                        ...
                                        A.insumos_total,
                                        coalesce(sum(c.cantidad_solicitada),0)    as insumos_solicitados,
                                        coalesce((a.insumos_total - sum(c.cantidad_solicitada)),insumos_total)  as insumos_disponibles,
                                ...
                                left join 	opav.sl_orden_compra_servicio  AS 	D ON (C.id_ocs = D.id and D.cod_ocs  not ilike '%-%')
                                WHERE
                                        A.responsable ilike _usuario
                                        and A.id_solicitud = _idsolicitud
                                        and A.id_solicitud_ocs = 0
                                        and cod_solicitud_devolucion = ''
...
ALTER FUNCTION opav.sp_listarinsumos_solicitudocs_ricardo2(character varying, character varying, character varying, character varying)
  OWNER TO postgres;
