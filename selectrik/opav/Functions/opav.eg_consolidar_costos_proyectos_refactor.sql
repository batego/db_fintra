-- Function: opav.eg_consolidar_costos_proyectos_refactor()

-- DROP FUNCTION opav.eg_consolidar_costos_proyectos_refactor();

CREATE OR REPLACE FUNCTION opav.eg_consolidar_costos_proyectos_refactor()
...
BEGIN
 ...
 TRUNCATE TABLE business_intelligence.reporte_consolidado_costos_proyectos2 ;
 FOR recordReporte IN (SELECT  num_os,
			...
			WHERE num_os !='' and num_os='FOMS11234-16'
			)
 LOOP
...
-- 	Inymec
        INSERT INTO business_intelligence.reporte_consolidado_costos_proyectos2 (empresa, tipo_referencia, multiservicio,id_solicitud, proveedor, nombre_proveedor, fecha_documento, tipo_documento, documento, descripcion, valor_antes_iva,
	...
	FROM opav.eg_costos_inymec_consolidado(recordReporte.num_os);
	...
	SELECT
		...
		recordReporte.tipo_trabajo,
		recordReporte.tipo_negocio
	FROM opav.eg_costos_provintegral_consolidado(recordReporte.id_solicitud);
	...
	SELECT
		...
		recordReporte.tipo_trabajo,
		recordReporte.tipo_negocio
	FROM opav.eg_costos_fintra_consolidado(recordReporte.num_os);
...
-- 	Selectrik
	...
	SELECT  empresa,
		...
		recordReporte.tipo_trabajo,
		recordReporte.tipo_negocio
	FROM opav.eg_costos_selectrik_consolidado(recordReporte.num_os);
...
ALTER FUNCTION opav.eg_consolidar_costos_proyectos_refactor()
  OWNER TO postgres;
