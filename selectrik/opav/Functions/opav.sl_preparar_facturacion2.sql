-- Function: opav.sl_preparar_facturacion2(character varying, numeric, numeric, character varying, text)

-- DROP FUNCTION opav.sl_preparar_facturacion2(character varying, numeric, numeric, character varying, text);

CREATE OR REPLACE FUNCTION opav.sl_preparar_facturacion2(_id_solicitud character varying, _valor_facturar numeric, _valor_fact_material numeric, usuariocrea character varying, observaciones text)
  RETURNS text AS
$BODY$
...
BEGIN
	--select  opav.sl_preparar_facturacion2('924776' , 3723357 , 1000000 , 'WSIADO' , 'LOL');
	...
	ELSE
		...
		INSERT INTO
			opav.acciones(id_accion, id_solicitud, estado, contratista, material, mano_obra, administracion , imprevisto , utilidad , descripcion, tipo_trabajo,preparar_facturacion,prefacturar)
		VALUES
			(_consecutivoAccion,_id_solicitud,'090','CC761',_valor_fact_material/recordDistribucion.porcentaje, _valor_mano_obra/recordDistribucion.porcentaje, (_costo_contratista * _recordcot.administracion/100) , (_costo_contratista * _recordcot.imprevisto/100 ), (_costo_contratista * _recordcot.utilidad/100 ),'Mano de obra', 'Proyectos','S','N');
		...
		VALUES
			('',nextval('opav.cotizaciondets_idcotizaciondets_seq'),'PR035406',1.0,'N',_consecutivoCotizacion,'now()',
			'',_consecutivoAccion,'N',1.0,_valor_facturar/recordDistribucion.porcentaje, round(((_valor_facturar * ((_recordcot.perc_aiu/100)+1)) + (_valor_facturar* (_recordcot.perc_utilidad/100) * (_iva-1)) ),2) ,'');
	END IF;
...
ALTER FUNCTION opav.sl_preparar_facturacion2(character varying, numeric, numeric, character varying, text)
  OWNER TO postgres;
