-- Function: opav.sp_eliminarocs(character varying, character varying)

-- DROP FUNCTION opav.sp_eliminarocs(character varying, character varying);

CREATE OR REPLACE FUNCTION opav.sp_eliminarocs(_codocs character varying, _users character varying)
	...
	IF ( result.respta = 'POSITIVO' ) THEN
		...
		IF ( FOUND ) THEN
			...
			IF ( FOUND ) THEN
				...
				DELETE FROM opav.sl_ocs_detalle WHERE id_ocs in (select id from opav.sl_orden_compra_servicio where cod_ocs = _CodOCS);
				IF ( FOUND ) THEN
					raise notice 'E';
...
ALTER FUNCTION opav.sp_eliminarocs(character varying, character varying)
  OWNER TO postgres;
