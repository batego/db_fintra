-- Function: opav.eg_costos_inymec_consolidado(character varying)

-- DROP FUNCTION opav.eg_costos_inymec_consolidado(character varying);

CREATE OR REPLACE FUNCTION opav.eg_costos_inymec_consolidado(_multiservicio character varying)
	...
	FOR recordCdiar IN (SELECT  tipo_referencia_1,
				...
				FROM dblink('dbname=inymec
					...
					'SELECT
						...
						sum(cd.valor_debito) AS valor_comprobante,
						'''' as cuenta,
						'''' as nombre_largo
...
ALTER FUNCTION opav.eg_costos_inymec_consolidado(character varying)
  OWNER TO postgres;
