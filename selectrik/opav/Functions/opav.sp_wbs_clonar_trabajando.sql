-- Function: opav.sp_wbs_clonar_trabajando(integer, integer, character varying)

-- DROP FUNCTION opav.sp_wbs_clonar_trabajando(integer, integer, character varying);

CREATE OR REPLACE FUNCTION opav.sp_wbs_clonar_trabajando(_id_solicitud_viejo integer, _id_solicitud_nuevo integer, _usuario character varying)
...
BEGIN
	-- SELECT opav.sp_wbs_clonar(924241,924668, 'WSIADO');
            ...
            LOOP
                  ...
                  LOOP
                        ...
                        LOOP
                              ...
                              INSERT INTO opav.sl_actividades
						(descripcion, creation_date, creation_user)
                              ...
                              INSERT INTO opav.sl_actividades_capitulos
						(id_capitulo, id_actividad, creation_date, creation_user)
				...
				_wast:=0;
                              FOR _sl_rel_actividades_apu IN
                                    ...
                                    INNER JOIN 	OPAV.SL_APU 			AS B			ON (A.ID_APU = B.ID)
                                    INNER JOIN  OPAV.SL_APU_DET 		AS C			ON (C.ID_APU = B.ID )
                                    WHERE
						A.id_actividad_capitulo 	= 	_sl_actividades_capitulos.id_capitulos_disciplinas_viejo
						AND A.reg_status 		= 	''
						AND B.reg_status 		= 	''
						AND C.reg_status 		= 	''
...
ALTER FUNCTION opav.sp_wbs_clonar_trabajando(integer, integer, character varying)
  OWNER TO postgres;
