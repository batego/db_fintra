-- Function: opav.sl_get_valor_disciplina(integer)

-- DROP FUNCTION opav.sl_get_valor_disciplina(integer);

CREATE OR REPLACE FUNCTION opav.sl_get_valor_disciplina(id_area_ integer)
  RETURNS text AS
$BODY$
...
SELECT da.id,
            ...
            where
            /*a.id_solicitud=ap.id_solicitud and b.id=ap.id*/ c.id=da.id and f.reg_status='' and e.reg_status=''),'LFM9,999,999,999,999') as grouptitle,
            ...
            WHERE id_area_proyecto in(id_area_) AND da.reg_status = '' order by id_area_proyecto,da.id;
...
ALTER FUNCTION opav.sl_get_valor_disciplina(integer)
  OWNER TO postgres;
