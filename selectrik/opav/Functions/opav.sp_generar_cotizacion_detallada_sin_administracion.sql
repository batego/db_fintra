-- Function: opav.sp_generar_cotizacion_detallada_sin_administracion(character varying)

-- DROP FUNCTION opav.sp_generar_cotizacion_detallada_sin_administracion(character varying);

CREATE OR REPLACE FUNCTION opav.sp_generar_cotizacion_detallada_sin_administracion(id_solicitud_ character varying)
	...
	FOR cotizacion IN
				select
					...
					((i.valor_esquema/i.cantidad_insumo/i.rendimiento_insumo/i.cantidad_apu)* (
									...
									END )
									--*_cotos_indirectos
									)::numeric(19,3) as valor_unitario,
...
ALTER FUNCTION opav.sp_generar_cotizacion_detallada_sin_administracion(character varying)
  OWNER TO postgres;
