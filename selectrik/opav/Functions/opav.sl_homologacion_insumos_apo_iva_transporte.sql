-- Function: opav.sl_homologacion_insumos_apo_iva_transporte(integer, integer)

-- DROP FUNCTION opav.sl_homologacion_insumos_apo_iva_transporte(integer, integer);

CREATE OR REPLACE FUNCTION opav.sl_homologacion_insumos_apo_iva_transporte(id_solicitud_ integer, id_tipo_insumo_ integer)
  RETURNS record AS
$BODY$
DECLARE
...
BEGIN
	--select opav.sl_homologacion_insumos_apo_iva_transporte(923780 , 5 )
...
ALTER FUNCTION opav.sl_homologacion_insumos_apo_iva_transporte(integer, integer)
  OWNER TO postgres;
