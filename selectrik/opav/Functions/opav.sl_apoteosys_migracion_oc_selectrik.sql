-- Function: opav.sl_apoteosys_migracion_oc_selectrik(character varying)

-- DROP FUNCTION opav.sl_apoteosys_migracion_oc_selectrik(character varying);

CREATE OR REPLACE FUNCTION opav.sl_apoteosys_migracion_oc_selectrik(oc_ character varying)
...
BEGIN
	...
	select case when oc_ ilike '%OC%' then 1 ELSE 2 END INTO _tipo_orden;
	...
	IF(_existe = 'NO' )THEN
		...
		LOOP
			...
			_rec_sum :=(select opav.sl_homologacion_insumos_apo(_ocs.id_solicitud::integer , _ocs.id_insumo , _tipo_orden::integer));
...
ALTER FUNCTION opav.sl_apoteosys_migracion_oc_selectrik(character varying)
  OWNER TO postgres;
