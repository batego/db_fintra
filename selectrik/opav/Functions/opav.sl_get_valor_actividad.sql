-- Function: opav.sl_get_valor_actividad(integer, integer)

-- DROP FUNCTION opav.sl_get_valor_actividad(integer, integer);

CREATE OR REPLACE FUNCTION opav.sl_get_valor_actividad(id_cap_ integer, id_act_ integer)
  RETURNS text AS
$BODY$
...
begin
   ...
   select          (select
                ...
                where
                /*a.id_solicitud=ap.id_solicitud and*/ e.id=ac.id and e.id_actividad=ac.id_actividad and f.reg_status='' and e.reg_status='') as valor_act into retorno
...
ALTER FUNCTION opav.sl_get_valor_actividad(integer, integer)
  OWNER TO postgres;
