-- Function: opav.sl_get_valor_apu(integer, integer, integer, integer)

-- DROP FUNCTION opav.sl_get_valor_apu(integer, integer, integer, integer);

CREATE OR REPLACE FUNCTION opav.sl_get_valor_apu(id_pr integer, id_cap_ integer, id_act_ integer, id_apu_ integer)
  RETURNS text AS
$BODY$
...
begin
    ...
    select	sum((costo_personalizado*cantidad_insumo*rendimiento_insumo*cantidad_apu))::numeric(19,3) as total1 into retorno
...
ALTER FUNCTION opav.sl_get_valor_apu(integer, integer, integer, integer)
  OWNER TO postgres;
