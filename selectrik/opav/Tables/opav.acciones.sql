...
-- DROP TABLE opav.acciones;

CREATE TABLE opav.acciones
(
  ...
  accion_principal character varying(1) NOT NULL DEFAULT '0'::character varying,
  preparar_facturacion character varying(1) NOT NULL DEFAULT 'N'::character varying,
  facturado character varying(1) NOT NULL DEFAULT 'N'::character varying
)
WITH (
  OIDS=FALSE
);
ALTER TABLE opav.acciones
  OWNER TO postgres;
COMMENT ON COLUMN opav.acciones.material IS 'costo contratista';
COMMENT ON COLUMN opav.acciones.administracion IS 'costo contratista';
COMMENT ON COLUMN opav.acciones.tipo_trabajo IS 'parece ser solo informativo';
COMMENT ON COLUMN opav.acciones.exestado IS 'columna temporal';
COMMENT ON COLUMN opav.acciones.fi_provintegral IS 'Numero de factura inerna para Provintegral';
COMMENT ON COLUMN opav.acciones.fc_provintegral IS 'Numero de factura conformada para Provintegral';
COMMENT ON COLUMN opav.acciones.ff_provintegral IS 'Fecha factura conformada para Provintegral';
COMMENT ON COLUMN opav.acciones.nc_provintegral IS 'Nota credito que cancela la factura interna de provintegral para trasladarla a Fintra';
COMMENT ON COLUMN opav.acciones.ft_provintegral IS 'Fecha traslado de la factura conformada a Fintra';
COMMENT ON COLUMN opav.acciones.fc_fintra IS 'Numero de factura conformada para Fintra';
COMMENT ON COLUMN opav.acciones.fc_opav IS 'Numero de factura conformada para Opav';


-- Trigger: hacciones on opav.acciones

-- DROP TRIGGER hacciones ON opav.acciones;

CREATE TRIGGER hacciones
  AFTER INSERT OR UPDATE
  ON opav.acciones
  FOR EACH ROW
  EXECUTE PROCEDURE insert_h_acc();
