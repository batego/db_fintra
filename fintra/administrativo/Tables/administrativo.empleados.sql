-- Table: administrativo.empleados

-- DROP TABLE administrativo.empleados;
...
(
  ...
  id_producto integer,
  id_rel_proceso_ccosto integer,
  tercerizado character varying(1) NOT NULL DEFAULT 'N'::character varying,
  tercero character varying(10),
  ...
  CONSTRAINT empleados_ciudad_expedicion_fkey FOREIGN KEY (ciudad_expedicion)
  ...
  CONSTRAINT empleados_departamento_fkey FOREIGN KEY (departamento)
      ...
      ON UPDATE NO ACTION ON DELETE NO ACTION,
  CONSTRAINT empleados_dpto_expedicion_fkey FOREIGN KEY (dpto_expedicion)
      REFERENCES estado (department_code) MATCH SIMPLE
      ON UPDATE NO ACTION ON DELETE NO ACTION,
  ...
  CONSTRAINT empleados_id_proceso_meta_fkey FOREIGN KEY (id_proceso_meta)
      ...
      ON UPDATE NO ACTION ON DELETE NO ACTION,
  CONSTRAINT empleados_id_producto_fkey FOREIGN KEY (id_producto)
      REFERENCES administrativo.productos (id) MATCH SIMPLE
      ON UPDATE NO ACTION ON DELETE NO ACTION,
  CONSTRAINT empleados_id_profesion_fkey FOREIGN KEY (id_profesion)
      REFERENCES administrativo.profesiones (id) MATCH SIMPLE
      ON UPDATE NO ACTION ON DELETE NO ACTION,
  CONSTRAINT empleados_id_rel_proceso_ccosto_fkey FOREIGN KEY (id_rel_proceso_ccosto)
      REFERENCES rel_procesometa_centrocosto (id) MATCH SIMPLE
      ON UPDATE NO ACTION ON DELETE NO ACTION,
...
ALTER TABLE administrativo.empleados
  OWNER TO postgres;

-- Trigger: dv_insert_empleado_pasivo on administrativo.empleados

-- DROP TRIGGER dv_insert_empleado_pasivo ON administrativo.empleados;

CREATE TRIGGER dv_insert_empleado_pasivo
  AFTER INSERT
  ON administrativo.empleados
  FOR EACH ROW
  EXECUTE PROCEDURE dv_insert_empleado_pasivo();
