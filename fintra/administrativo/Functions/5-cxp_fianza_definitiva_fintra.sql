-- Function: administrativo.cxp_fianza_definitiva_fintra(integer, character varying, character varying, text[], numeric, character varying, character varying)

-- DROP FUNCTION administrativo.cxp_fianza_definitiva_fintra(integer, character varying, character varying, text[], numeric, character varying, character varying);

CREATE OR REPLACE FUNCTION administrativo.cxp_fianza_definitiva_fintra(_unidad_negocio integer, empresafianza character varying, periodo_corte character varying, items_cxp text[], vlr_cxp numeric, usuario character varying, _agencia character varying)
  RETURNS text AS
$BODY$
DECLARE
...
BEGIN
	...
	END IF;
            ...
            RAISE NOTICE '_IDCONVENIO: %',_IDCONVENIO;
            RAISE NOTICE 'CODIGO_NC_FIANZA_TEMP: %',codigo_nc_fianza_temp;
            RAISE NOTICE 'CODIGO_CXP_FIANZA_DEF: %',codigo_cxp_fianza_def;
       ...
       --VALIDAMOS EL PERFIL CONTABLE PARA CREAR LA CXP DE FIANZA--
		...
		WHERE nit = empresaFianza;
			...
			LEFT JOIN fin.cxp_doc cp ON (cp.documento=df.documento_relacionado and cp.reg_status='' and cp.proveedor=df.nit_empresa_fianza  and cp.dstrct='FINV' and cp.tipo_documento='FAP')
			WHERE id = ANY (items_cxp::int[]) AND df.reg_status='';

			raise notice 'Cabecera cxp creada';
	...
	raise notice 'detalle cxp creada';
...
ALTER FUNCTION administrativo.cxp_fianza_definitiva_fintra(integer, character varying, character varying, text[], numeric, character varying, character varying)
  OWNER TO postgres;
