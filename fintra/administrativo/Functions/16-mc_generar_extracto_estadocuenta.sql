-- Function: administrativo.mc_generar_extracto_estadocuenta(character varying, date, character varying)

-- DROP FUNCTION administrativo.mc_generar_extracto_estadocuenta(character varying, date, character varying);

CREATE OR REPLACE FUNCTION administrativo.mc_generar_extracto_estadocuenta(negocio_ character varying, fecha_corte date, usuario character varying)
  RETURNS text AS
$BODY$
DECLARE
...
UnidadNeg integer:=0;
retorno varchar:='';
conceptos_ varchar[]= array['SEG','CAP','INT','IXM','GAC','CAT'];
	...
	loop
		...
		if (id_concepto_rec.descrip !='')then
			...
			if (conceptos_[i] ='CAT')then
				...
				RAISE NOTICE 'valor_concepto cat %',valor_concepto;
			end if;
...
ALTER FUNCTION administrativo.mc_generar_extracto_estadocuenta(character varying, date, character varying)
  OWNER TO postgres;
