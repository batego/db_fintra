-- Function: business_intelligence.eg_analisis_recaudo()

-- DROP FUNCTION business_intelligence.eg_analisis_recaudo();

CREATE OR REPLACE FUNCTION business_intelligence.eg_analisis_recaudo()
  RETURNS SETOF business_intelligence.rs_datos_recaudo_v2 AS
$BODY$

DECLARE

 rs business_intelligence.rs_datos_recaudo_v2;
 recordPagos record ;
 recordNeg record;
 _aux VARCHAR:='';
 ...
 cursor_recaudo CURSOR FOR (SELECT    substring(idet.periodo,1,4) as anio,
				      idet.periodo,
				...
				--AND idet.num_ingreso in ('IC248337')
...
BEGIN
	...
	LOOP
		...
		SELECT INTO recordNeg un.descripcion, neg.cod_neg, c.agencia FROM con.factura fac
		...
		WHERE documento=recordPagos.documento AND tipo_documento IN ('FAC','NDC') AND un.id in (1,2,3,4,6,7,8,9,10,21,22,31);

		RAISE NOTICE 'recordNeg.descripcion % recordNeg.agencia %',recordNeg.descripcion,recordNeg.agencia;
		IF(_aux='')THEN
...
ALTER FUNCTION business_intelligence.eg_analisis_recaudo()
  OWNER TO postgres;
