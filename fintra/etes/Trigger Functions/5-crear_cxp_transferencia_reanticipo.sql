-- Function: etes.crear_cxp_transferencia_reanticipo()

-- DROP FUNCTION etes.crear_cxp_transferencia_reanticipo();

CREATE OR REPLACE FUNCTION etes.crear_cxp_transferencia_reanticipo()
  RETURNS "trigger" AS
$BODY$
DECLARE
...
cmc_factura varchar:='' ;
contabilizarCXP varchar:='' ;

BEGIN

	--ANTICIPO TRANSFERENCIA
	RAISE NOTICE 'FUNCTION CREAR CXP TRANSFERENCIA REANTICIPO';
		...
		IF(etes.validacion_cuentas('CXP_TRANSFERENCIA'))THEN
			...
			FOR recordManifiesto IN(
						    ...
						    WHERE
						    anticipo.reg_status=''
						    and reanticipo.id=NEW.id
						    and reanticipo.reg_status=''
						    and reanticipo.transferido='N'
						    and reanticipo.fecha_transferencia='0099-01-01 00:00:00'::timestamp without time zone
						    and reanticipo.aprobado='N'
						    and reanticipo.fecha_aprobacion ='0099-01-01 00:00:00'::timestamp without time zone
			...
			LOOP
				...
				IF(recordManifiesto.codigo_proserv ='ANT00002' AND OLD.valor_desembolsar = 0 AND NEW.valor_desembolsar > 0) THEN
					...
					RAISE NOTICE 'FUNCTION CXP CODPRODUCTO:: %',recordManifiesto.codigo_proserv;
					...
					--B)ACTUALIZO EL MANIFIESTO REANTICIPO CON EL NUMERO DE CXP
...
ALTER FUNCTION etes.crear_cxp_transferencia_reanticipo()
  OWNER TO postgres;
