-- Table: etes.propietario

-- DROP TABLE etes.propietario;

CREATE TABLE etes.propietario
(
  id serial NOT NULL,
  ...
  CONSTRAINT propietario_cod_proveedor_check CHECK (cod_proveedor::text <> ''::text AND cod_proveedor::text <> ' '::text AND cod_proveedor::text <> '  '::text),
  CONSTRAINT propietario_nombre_check CHECK (nombre::text <> ''::text AND nombre::text <> ' '::text AND nombre::text <> '  '::text)
)
WITH (
  OIDS=FALSE
);
ALTER TABLE etes.propietario
  OWNER TO postgres;
