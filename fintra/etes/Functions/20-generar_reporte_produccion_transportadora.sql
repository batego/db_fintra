-- Function: etes.generar_reporte_produccion_transportadora()

-- DROP FUNCTION etes.generar_reporte_produccion_transportadora();

CREATE OR REPLACE FUNCTION etes.generar_reporte_produccion_transportadora()
  RETURNS SETOF etes.rs_reporte_produccion_trans AS
$BODY$
		DECLARE

		 recordAnticipos record;
		 recordDinamico record;
		 rs etes.rs_reporte_produccion_trans;
			...
			LOOP
				...
				************************************************/
				...
				FOR recordReanticipo IN (SELECT
								...
								,reanticipo.origen
								,reanticipo.destino
								,reanticipo.fecha_pago_fintra
								,reanticipo.cxc_corrida as nro_corrida
							    FROM  etes.manifiesto_reanticipos  AS reanticipo
								LEFT JOIN etes.manifiesto_carga  AS anticipo ON (reanticipo.id_manifiesto_carga= anticipo.id AND anticipo.reg_status='' )
				...
				LOOP
					...
					******************************************************************/
					...
					rs.destino :=recordReanticipo.destino;
                                        rs.fecha_pago_fintra :=recordReanticipo.fecha_pago_fintra ;
...
ALTER FUNCTION etes.generar_reporte_produccion_transportadora()
  OWNER TO postgres;
