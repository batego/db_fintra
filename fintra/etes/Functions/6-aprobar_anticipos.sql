-- Function: etes.aprobar_anticipos(text[], character varying)

-- DROP FUNCTION etes.aprobar_anticipos(text[], character varying);

CREATE OR REPLACE FUNCTION etes.aprobar_anticipos(anticipos text[], usuario character varying)
  RETURNS boolean AS
$BODY$
DECLARE
	...
	LOOP
		...
		IF(anticipos[i][12]='N')THEN
			...
			UPDATE etes.manifiesto_carga
			   SET aprobado='S', fecha_aprobacion=NOW(),
...
ALTER FUNCTION etes.aprobar_anticipos(text[], character varying)
  OWNER TO postgres;
