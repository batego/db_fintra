-- Function: etes.descontar_comisiones_xventas(character varying, integer)

-- DROP FUNCTION etes.descontar_comisiones_xventas(character varying, integer);

CREATE OR REPLACE FUNCTION etes.descontar_comisiones_xventas(numerocxp character varying, id_estacion integer)
  RETURNS text AS
$BODY$
...
BEGIN
	...
	/*SELECT  into count_cxp_generadas  count(0) FROM fin.cxp_doc
	WHERE documento LIKE 'EDS%' AND tipo_documento='FAP'
	AND handle_code='EG' AND dstrct='FINV' AND reg_status=''
	AND REPLACE(SUBSTRING(creation_date,1,7),'-','')=REPLACE(SUBSTRING(NOW(),1,7),'-','');*/
	...
	Select into count_cxp_generadas count(0) from (
		...
		WHERE documento LIKE 'EDS%' AND tipo_documento='FAP'
		AND handle_code='EG' AND cxp.dstrct='FINV' AND cxp.reg_status='' and ventas.id_eds=id_estacion
		AND REPLACE(SUBSTRING(cxp.creation_date,1,7),'-','')=REPLACE(SUBSTRING(NOW(),1,7),'-','')
	...
	IF(count_cxp_generadas = 1)THEN
		...
		--1.1) Buscamos el perido de la facturas del mes anterior.
			...
			IF (replace(substring((NOW()::date) ,5,3),'-','')='01') THEN
			    perido_corte:=(replace(substring((NOW()::date) ,1,4),'-','')::numeric)-1||'12'::NUMERIC;
			ELSE
			    perido_corte:=replace(substring(NOW(),1,7),'-','')::numeric - 1;
			END IF;
...
ALTER FUNCTION etes.descontar_comisiones_xventas(character varying, integer)
  OWNER TO postgres;
