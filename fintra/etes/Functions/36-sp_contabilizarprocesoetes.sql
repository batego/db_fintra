-- Function: etes.sp_contabilizarprocesoetes(integer, character varying, character varying, integer)

-- DROP FUNCTION etes.sp_contabilizarprocesoetes(integer, character varying, character varying, integer);

CREATE OR REPLACE FUNCTION etes.sp_contabilizarprocesoetes(_idmanifiesto integer, tipo_anticipo character varying, usuario character varying, _idestacion integer)
  RETURNS text AS
$BODY$
...
BEGIN
	...
	**/
	...
	FOR InfoManfts IN
		...
		--mc.fecha_corrida::date = '2015-09-03' and
		mc.cxc_corrida = '' and
		mc.reg_status = ''
		order by mc.id
		--InfoManfts.identificacion
	LOOP
		...
		--B) GENERO LA CXP AL PROPIETARIO
			...
			INSERT INTO fin.cxp_doc(
			    ...
			    VALUES ('', 'FINV', InfoManfts.cod_proveedor,  'FAP', InfoManfts.planilla, 'TIPO OPERACION : AGA   PLANILLA: '||InfoManfts.planilla,
				    'OP', 'IL', '', '', '',
				    '0099-01-01 00:00:00'::timestamp, 'JGOMEZ', '', 'ESTACION', 'GASOLINA',
		...
		--C) GENERO EL EGRESO AL PROPIETARIO
			...
			INSERT INTO con.comprodet(
			    ...
			    VALUES ('', 'FINV', 'EGR', EGRPropietario, _grupo_transaccion, _transaccion,
				    _PeriodoCte, '22050411', InfoManfts.cod_proveedor, 'PROPIETARIO '||InfoManfts.cod_proveedor, InfoManfts.valor_neto_anticipo, 0.00,
			...
			INSERT INTO con.comprodet(
				    ...
				    tipo_referencia_3, referencia_3)
			    VALUES ('', 'FINV', 'EGR', EGRPropietario, _grupo_transaccion, _transaccion,
				    _PeriodoCte, '23050313', InfoManfts.cod_proveedor, 'ESTACION-GASOLINA '||RsEstacion.nit_estacion, 0.00, InfoManfts.valor_neto_anticipo,
				...
				IF FOUND THEN
					...
					INSERT INTO con.comprobante(
					    ...
					    VALUES ('', 'FINV', 'FAC', CXCPropietario, _grupo_transaccion, 'OP',
						    _PeriodoCte, now()::date, 'CONT FAC '||CXCPropietario, InfoManfts.cod_proveedor, InfoManfts.valor_desembolsar, InfoManfts.valor_desembolsar,
						    2, 'PES', '0099-01-01 00:00:00', Usuario, now(),
						    Usuario, now(), Usuario, 'COL', Usuario,
						    '003', '', 0.00, '', '');
...
ALTER FUNCTION etes.sp_contabilizarprocesoetes(integer, character varying, character varying, integer)
  OWNER TO postgres;
