-- Function: fin.dv_archivo_facturacion_efecty(text[], text, integer)

-- DROP FUNCTION fin.dv_archivo_facturacion_efecty(text[], text, integer);

CREATE OR REPLACE FUNCTION fin.dv_archivo_facturacion_efecty(_cabecera text[], _periodo text, _ciclo integer)
...
--select to_char(current_timestamp, 'YYYYMMDDHH24MI') (replace(replace(replace(now(),'-',''),':',''),' ',''))
  ...
  FOR _record_det IN (SELECT
			    ...
			    INNER JOIN con.ciclos_facturacion ciclo
				    ...
				    WHERE duplicado = 'N' and total <= 500000.00)
...
ALTER FUNCTION fin.dv_archivo_facturacion_efecty(text[], text, integer)
  OWNER TO postgres;
