-- Function: con.interfaz_automotor_aval_apoteosys(character varying, character varying, character varying, character varying, date)

-- DROP FUNCTION con.interfaz_automotor_aval_apoteosys(character varying, character varying, character varying, character varying, date);

CREATE OR REPLACE FUNCTION con.interfaz_automotor_aval_apoteosys(negocio_padre character varying, secuencia_g character varying, tipo_doc_ap character varying, clase_documento character varying, fecha_padre date)
  RETURNS text AS
$BODY$
...
DECLARE
 ...
 /************************************************
  *DESCRIPCION: ESTA FUNCION BUSCA TODOS LO NEGOCIOS AVALES AUTOMOTOR.
  *AUTOR:=@MMEDINA
...
CUENTAS_ VARCHAR[] := '{13050902,27050901,28150602}';
FECHADOC_ VARCHAR:= '';
...
BEGIN
	...
	FOR NEGOCIO_ IN
		...
		where neg.estado_neg in ('T','A') and uneg.id = '13'
		and procesado_mc = 'N'
	...
	LOOP
		...
		LOOP
			IF(CON.OBTENER_HOMOLOGACION_APOTEOSYS('AUTOMOTOR',INFOITEMS_.tipo_documento, INFOITEMS_.cuenta,NEGOCIO_.agencia, 6)='S')THEN
			...
			MCTYPE.MC_____CODIGO____CPC____B := CON.OBTENER_HOMOLOGACION_APOTEOSYS('AUTOMOTOR', INFOITEMS_.tipo_documento, INFOITEMS_.cuenta, NEGOCIO_.agencia, 1);
			MCTYPE.MC_____CODIGO____CU_____B := CON.OBTENER_HOMOLOGACION_APOTEOSYS('AUTOMOTOR', INFOITEMS_.tipo_documento, INFOITEMS_.cuenta, NEGOCIO_.agencia, 2);
			...
			MCTYPE.MC_____CODIGO____DS_____B := CON.OBTENER_HOMOLOGACION_APOTEOSYS('AUTOMOTOR',INFOITEMS_.tipo_documento, INFOITEMS_.cuenta, NEGOCIO_.agencia, 3);
			IF(CON.OBTENER_HOMOLOGACION_APOTEOSYS('AUTOMOTOR', INFOITEMS_.tipo_documento, INFOITEMS_.cuenta,NEGOCIO_.agencia, 4)='S')THEN
			...
			IF(CON.OBTENER_HOMOLOGACION_APOTEOSYS('AUTOMOTOR', INFOITEMS_.tipo_documento, INFOITEMS_.cuenta, NEGOCIO_.agencia, 5)::INT=1)THEN
		...
		IF CON.SP_VALIDACIONES(MCTYPE,'AUTOMOTOR') ='N' THEN
			SW = 'N';
...
ALTER FUNCTION con.interfaz_automotor_aval_apoteosys(character varying, character varying, character varying, character varying, date)
  OWNER TO postgres;
