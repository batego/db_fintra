...
BEGIN
	...
	FOR EGRESO_RECORD IN
			...
			--query principal
				...
				WHERE F.TIPO_DOCUMENTO = 'FAC'
				--AND F.CMC = 'PO'
				  AND PERIODO = '201803'
			        --AND REPLACE(SUBSTRING(F.CREATION_DATE,1,7),'-','') ='201701'
				  AND F.NIT='9000742958'
				  AND F.REG_STATUS = ''
				  --AND F.DOCUMENTO = 'R0032227'
			...
			--ELIMINAR REGISTROS DE TABLA TRASLADO
			--DELETE FROM CON.MC_EGRE_PROV_PROVINT WHERE PROCESADO = 'N' AND CREATION_DATE = '2018-03-02 11:54:34.697548'

			--DESMARCAR DOCUMENTOS QUE FUERON PASADOSA TABLA DE TRASLADO
			--UPDATE CON.FACTURA SET PROCESADO = 'N' WHERE PROCESADO = 'S'  AND PERIODO = '201701' AND DOCUMENTO = 'R0032227' AND TIPO_DOCUMENTO = 'FAC';
	...
	LOOP
		...
		FOR INFOITEMS_ IN
					...
					WHERE DOCUMENTO = EGRESO_RECORD.DOCUMENTO --R0032227' --
					 AND TIPO_DOCUMENTO = EGRESO_RECORD.TIPO_DOCUMENTO --'FAC'  --
					 AND FAC.REG_STATUS = ''
					 AND NIT = EGRESO_RECORD.NIT --'9000742958'      --
...
--
			...
			IF(CON.OBTENER_HOMOLOGACION_APOTEOSYS('EGR_FIN','FAP', INFOITEMS_.CUENTA,'', 6)='S')THEN
			...
			MCTYPE.MC_____CODIGO____CPC____B := CON.OBTENER_HOMOLOGACION_APOTEOSYS('EGR_FIN', 'FAP', INFOITEMS_.CUENTA,'', 1);
			MCTYPE.MC_____CODIGO____CU_____B := CON.OBTENER_HOMOLOGACION_APOTEOSYS('EGR_FIN', 'FAP', INFOITEMS_.CUENTA,'', 2);
			...
			MCTYPE.MC_____CODIGO____DS_____B := CON.OBTENER_HOMOLOGACION_APOTEOSYS('EGR_FIN','FAP', INFOITEMS_.CUENTA,'', 3);
...
-- 			END IF;
			...
			IF(CON.OBTENER_HOMOLOGACION_APOTEOSYS('EGR_FIN', 'FAP', INFOITEMS_.CUENTA,'', 5)::INT=1)THEN
		...
		IF(SW = 'S')THEN
			...
			WHERE
				...
				AND PERIODO = EGRESO_RECORD.PERIODO
				AND DESCRIPCION = EGRESO_RECORD.DESCRIPCION;
