-- Function: con.sp_trasladocoficolombianafintrafenalco()

-- DROP FUNCTION con.sp_trasladocoficolombianafintrafenalco();

CREATE OR REPLACE FUNCTION con.sp_trasladocoficolombianafintrafenalco()
  RETURNS text AS
$BODY$
...
BEGIN
	...
	FOR factura_items IN select * from tem.traslado_corfi_fintra_fenalco where valor_saldo > 0 LOOP

		SELECT INTO _numero_factura get_lcod('ICAC');
		cnt = 1;
		_prefix = factura_items.prefijo;

		if ( _prefix = 'PC' ) then
		     _prefixNew = 'DC';
		   elsif ( _prefix = 'PG' ) then
		     _prefixNew = 'DG';
		   elsif ( _prefix = 'PL' ) then
		     _prefixNew = 'DL';
		   elsif ( _prefix = 'PF' ) then
		     _prefixNew = 'DF';
		   elsif ( _prefix = 'ND' ) then
		     _prefixNew = 'DD';
		end if;
		...
		dstrct,
		tipo_documento,
		_prefixNew||substring(documento,3),
		nit,
		codcli,
		concepto,
		fecha_factura,
...
ALTER FUNCTION con.sp_trasladocoficolombianafintrafenalco()
  OWNER TO postgres;
