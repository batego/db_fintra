-- Function: con.interfaz_educativo_aval_apoteosys(character varying, character varying, character varying, character varying, date, date, character varying)

-- DROP FUNCTION con.interfaz_educativo_aval_apoteosys(character varying, character varying, character varying, character varying, date, date, character varying);

CREATE OR REPLACE FUNCTION con.interfaz_educativo_aval_apoteosys(negocio_padre character varying, secuencia_g character varying, tipo_doc_ap character varying, clase_documento character varying, fecha_padre date, fecha_vencimiento_ date, periodo_ character varying)
  RETURNS text AS
$BODY$
...
DECLARE
 ...
 /************************************************
  *DESCRIPCION: ESTA FUNCION BUSCA TODOS LO NEGOCIOS AVALES EDUCATIVOS.
  *AUTOR:=@MMEDINA
...
CUENTAS_ VARCHAR[] := '{13050902, 27050901,28150602,13050521}';
FECHADOC_ VARCHAR:= '';
...
BEGIN
	...
	FOR NEGOCIO_ IN
		...
		where neg.estado_neg in ('T','A') and uneg.id = '12'
		and procesado_mc = 'N'
	...
	LOOP
		...
		FOR INFOITEMS_ IN
			(select
				...
				fecha_vencimiento_::DATE AS fecha_vencimiento,
				periodo_ as periodo,
		...
		LOOP
			IF(INFOITEMS_.tipo_documento in ('FAC','FAP','NEG') AND CON.OBTENER_HOMOLOGACION_APOTEOSYS('EDUCATIVO',INFOITEMS_.tipo_documento, INFOITEMS_.cuenta,NEGOCIO_.agencia, 6)='S')THEN
			...
			MCTYPE.MC_____CODIGO____CPC____B := CON.OBTENER_HOMOLOGACION_APOTEOSYS('EDUCATIVO', INFOITEMS_.tipo_documento, INFOITEMS_.cuenta, NEGOCIO_.agencia, 1);
			MCTYPE.MC_____CODIGO____CU_____B := CON.OBTENER_HOMOLOGACION_APOTEOSYS('EDUCATIVO', INFOITEMS_.tipo_documento, INFOITEMS_.cuenta, NEGOCIO_.agencia, 2);
			...
			MCTYPE.MC_____CODIGO____DS_____B := CON.OBTENER_HOMOLOGACION_APOTEOSYS('EDUCATIVO',INFOITEMS_.tipo_documento, INFOITEMS_.cuenta, NEGOCIO_.agencia, 3);
			IF(CON.OBTENER_HOMOLOGACION_APOTEOSYS('EDUCATIVO', INFOITEMS_.tipo_documento, INFOITEMS_.cuenta,NEGOCIO_.agencia, 4)='S')THEN
			...
			IF(CON.OBTENER_HOMOLOGACION_APOTEOSYS('EDUCATIVO', INFOITEMS_.tipo_documento, INFOITEMS_.cuenta, NEGOCIO_.agencia, 5)::INT=1)THEN
...
ALTER FUNCTION con.interfaz_educativo_aval_apoteosys(character varying, character varying, character varying, character varying, date, date, character varying)
  OWNER TO postgres;
