-- Function: con.interfaz_fintra_logistica_apoteosys_r()

-- DROP FUNCTION con.interfaz_fintra_logistica_apoteosys_r();

CREATE OR REPLACE FUNCTION con.interfaz_fintra_logistica_apoteosys_r()
  RETURNS text AS
$BODY$
...
--SE CONSULTAN LOS REGISTROS QUE SE VAN A TRANSPORTAR PARA EL BOTON DE INGRESO
	...
	FOR REC_OS IN
		...
		INNER JOIN FIN.CXP_DOC_TSP  CXP ON(CXP.DSTRCT='TSP' AND CXP.PROVEEDOR=EGRE.NIT AND CXP.DOCUMENTO=DETEGRE.DOCUMENTO)
		WHERE
		--CXP.PERIODO='201704'
		--AND
		CXP.REG_STATUS=''
		...
		AND EGRE.NUM_INGRESO IN('IC260141')
		GROUP BY
			EGRE.NUM_INGRESO,  CXP.CORRIDA
		ORDER BY
			EGRE.NUM_INGRESO
...
--SELECT con.interfaz_fintra_logistica_apoteosys_r();

/**286439091
SELECT COUNT(*) FROM con.mc____ where mc_____codigo____contab_b='FINT' AND procesado='N' AND MC_____CODIGO____TD_____B = 'CXCN'--AND mc_____codigo____cd_____b='AG'
SELECT * FROM con.mc____ where mc_____codigo____contab_b='FINT' AND procesado='N' AND MC_____CODIGO____TD_____B = 'CXCN'--AND mc_____codigo____cd_____b='AG'
delete FROM con.mc____ where mc_____codigo____contab_b='FINT' AND procesado='R' and mc_____secuinte__b in(3,4) AND MC_____CODIGO____TD_____B = 'CXCN' and mc_____codigo____cu_____b='A1111F21501'
...
UPDATE con.mc____ SET PROCESADO='N' where mc_____codigo____contab_b='FINT' and procesado='R'
*/
	...
	LOOP
			...
			/*FOR REC_R0 IN
				...
				WHERE  DOCUMENTO LIKE 'R0%'
					AND UPPER(DESCRIPCION) LIKE '%CORRIDA%'||REC_OS.CORRIDA||'%' AND CMC IN('DE','PD')
			LOOP
			...
			FOR REC_R2 IN
				...
				FROM
					CON.FACTURA
				WHERE  DOCUMENTO LIKE 'R0%'
				AND DESCRIPCION LIKE '%'||REC_OS.NUM_INGRESO||'%' AND CMC IN('TO','OT')
			...
			LOOP
				...
				IF(REC_R2.CMC='TO')THEN
					...
					LOOP
						...
						MCTYPE.MC_____CODIGO____CU_____B := CASE WHEN REC_R1.TIPO='AET' THEN 'A1111F21401'
						...
						MCTYPE.MC_____INDTIPMOV_B := 4  ;
						MCTYPE.MC_____INDMOVREV_B := 'N'  ;
						MCTYPE.MC_____OBSERVACI_B := REC_R1.DESCRIPCION  ;
			...
			LOOP
				...
				FOR REC_R5 IN
					...
					SELECT
						...
						--WHERE PLANILLA IN(SUBSTR(B.DESCRIPCION, 10, CHAR_LENGTH(B.DESCRIPCION)))
						WHERE PLANILLA IN (TRIM(SUBSTR(B.DESCRIPCION, position('PLANILLA' in B.DESCRIPCION)+9, CHAR_LENGTH(B.DESCRIPCION))))
						AND TIPO_OPERACION!='' limit 1) AS TIPO,
						SUBSTR(B.DESCRIPCION, position('PLANILLA' in B.DESCRIPCION)+9, CHAR_LENGTH(B.DESCRIPCION)) AS PLANILLA,
						B.DESCRIPCION
					FROM
						FIN.CXP_DOC A
					INNER JOIN
					...
					WHERE
						A.REG_STATUS='' AND
						A.DOCUMENTO=REC_R4.DOCUMENTO

				LOOP

					IF(REC_R4.TIPO_DOCUMENTO='FAP')THEN
					...
					MCTYPE.MC_____CODIGO____CU_____B := CASE WHEN REC_R5.TIPO='AET' THEN 'A1111F21401'
														WHEN REC_R5.TIPO='AGA' THEN 'A1111F21501'
														ELSE 'A1111F22201' END;
					...
					MCTYPE.MC_____INDTIPMOV_B := 4  ;
					MCTYPE.MC_____INDMOVREV_B := 'N'  ;
					MCTYPE.MC_____OBSERVACI_B := REC_R5.DESCRIPCION  ;
				...
				IF CON.SP_VALIDACIONES(MCTYPE,'LOGISTICA') ='N' THEN
					...
					IF(REC_R4.TIPO_DOCUMENTO='FAP')THEN
						DELETE FROM CON.MC____
						WHERE MC_____NUMERO____B = SECUENCIA_R AND MC_____CODIGO____CONTAB_B = 'FINT'
						 AND MC_____CODIGO____TD_____B = 'CXPN' AND  MC_____CODIGO____CD_____B = 'AG'  ;
					ELSIF(REC_R4.TIPO_DOCUMENTO='NC')THEN
						DELETE FROM CON.MC____
						WHERE MC_____NUMERO____B = SECUENCIA_R AND MC_____CODIGO____CONTAB_B = 'FINT'
...
ALTER FUNCTION con.interfaz_fintra_logistica_apoteosys_r()
  OWNER TO postgres;
