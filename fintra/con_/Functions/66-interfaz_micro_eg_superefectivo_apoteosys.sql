-- Function: con.interfaz_micro_eg_superefectivo_apoteosys()

-- DROP FUNCTION con.interfaz_micro_eg_superefectivo_apoteosys();

CREATE OR REPLACE FUNCTION con.interfaz_micro_eg_superefectivo_apoteosys()
  RETURNS text AS
$BODY$
...
BEGIN
	...
	FOR NOTAS_ IN
		...
		where
			eg.reg_status='' and
			eg.payment_name ='SUPEREFECTIVO SA' and
			eg.periodo>='201801' and
			egd.procesado='N'
		group by
			eg.dstrct,
			eg.branch_code,
		...
		--select con.interfaz_micro_eg_superefectivo_apoteosys()
		/**
		select COUNT(*) from con.mc_micro____ where MC_____CODIGO____TD_____B = 'CXPN' and MC_____CODIGO____CD_____B = 'NCSU' and procesado='N';
	...
	loop
		...
		loop
			...
			iF(CON.OBTENER_HOMOLOGACION_APOTEOSYS('NC_SUPER',INFOITEMS_.tipo_documento, INFOITEMS_.cuenta,INFOITEMS_.agencia, 6)='S')THEN
			...
			MCTYPE.MC_____CODIGO____CPC____B := CON.OBTENER_HOMOLOGACION_APOTEOSYS('NC_SUPER', INFOITEMS_.tipo_documento, INFOITEMS_.cuenta,INFOITEMS_.agencia, 1);
			MCTYPE.MC_____CODIGO____CU_____B := CON.OBTENER_HOMOLOGACION_APOTEOSYS('NC_SUPER', INFOITEMS_.tipo_documento, INFOITEMS_.cuenta,INFOITEMS_.agencia, 2);
			...
			MCTYPE.MC_____CODIGO____DS_____B := CON.OBTENER_HOMOLOGACION_APOTEOSYS('NC_SUPER',INFOITEMS_.tipo_documento, INFOITEMS_.cuenta,INFOITEMS_.agencia, 3);

			IF(CON.OBTENER_HOMOLOGACION_APOTEOSYS('NC_SUPER', INFOITEMS_.tipo_documento, INFOITEMS_.cuenta,INFOITEMS_.agencia, 4)='S')THEN
				MCTYPE.MC_____NUMDOCSOP_B := INFOITEMS_.doc_soporte;
			ELSE
				MCTYPE.MC_____NUMDOCSOP_B := '';
			END IF;

			IF(CON.OBTENER_HOMOLOGACION_APOTEOSYS('NC_SUPER', INFOITEMS_.tipo_documento, INFOITEMS_.cuenta,INFOITEMS_.agencia, 5)::INT=1)THEN
...
ALTER FUNCTION con.interfaz_micro_eg_superefectivo_apoteosys()
  OWNER TO postgres;
