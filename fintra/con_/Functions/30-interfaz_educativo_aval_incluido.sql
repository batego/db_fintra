-- Function: con.interfaz_educativo_aval_incluido()

-- DROP FUNCTION con.interfaz_educativo_aval_incluido();

CREATE OR REPLACE FUNCTION con.interfaz_educativo_aval_incluido()
  RETURNS text AS
$BODY$
...
BEGIN
	...
	FOR NEGOCIO_ IN
		select  neg.financia_aval,
			...
			sola.renovacion,
			conv.agencia,
			uneg.id as unidad_neg,replace(substring(neg.f_desem,1,7),'-','')
		from negocios neg
	...
	LOOP
		...
		FOR INFOITEMS_ IN
			...
			inner  JOIN (SELECT CODNEG,
					       ...
					       VALOR
					FROM ING_FENALCO WHERE REG_STATUS='' AND tipodoc='CM'
					ORDER BY
			...
			inner  JOIN (SELECT CODNEG,
					       ...
					       FECHA_DOC::DATE AS FECHA_VEN,
					       VALOR
					FROM ING_FENALCO WHERE REG_STATUS='' AND tipodoc='IF'
					ORDER BY
					COD,LPAD((SUBSTRING(COD,10,2)::INTEGER+1),2,'00')::INTEGER
				     ) B on (FAC.NEGASOC=B.CODNEG AND LPAD(SUBSTRING(FAC.DOCUMENTO,8,2),2,'00') =B.CUOTA)
			left join NIT D ON(D.CEDULA=fac.nit)
			...
			and proveedor not in ('8904800244','8901009858')
			and handle_code not in ('BA','AV')
			and substring (documento,1,2) in ('PM','PB')
			)union all
			(SELECT
				'aval' as descr,
				6::integer AS iteracion,
				1::varchar as doc,
				documento,
				'NEG' AS tipo_documento,
				CASE WHEN HT.NIT_APOTEOSYS IS NOT NULL THEN HT.NIT_APOTEOSYS ELSE proveedor END AS proveedor,
			...
			and vlr_neto > 0
			and cxp_doc.reg_status = ''
			and proveedor in ('8904800244','8901009858')
			and handle_code in ('BA','AV')
			and substring (documento,1,2) in ('PM','PB')
			)
		LOOP
			CUENTA_ASIENTO := con.interfaz_cuenta_diferidos_apoteosys(substring(NEGOCIO_.cod_neg,1,2)||'I', INFOITEMS_.iteracion, NEGOCIO_.agencia,NEGOCIO_.unidad_neg::varchar);
...
ALTER FUNCTION con.interfaz_educativo_aval_incluido()
  OWNER TO postgres;
