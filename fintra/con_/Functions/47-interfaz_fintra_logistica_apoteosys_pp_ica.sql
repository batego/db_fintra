-- Function: con.interfaz_fintra_logistica_apoteosys_pp_ica()

-- DROP FUNCTION con.interfaz_fintra_logistica_apoteosys_pp_ica();

CREATE OR REPLACE FUNCTION con.interfaz_fintra_logistica_apoteosys_pp_ica()
  RETURNS text AS
$BODY$
...
DECLARE
 ...
 /************************************************
  *DESCRIPCION: ESTA FUNCION TOMA LAS PP DE NOTAS DE AJUSTE Y
 ...
 FECHADOC_ TEXT:='';
 SW TEXT:='N';
 --_ARRCUENTASAET VARCHAR[] :='{28050508,13802602}';--IDEAL UN TABLA OJO!!
 _ARRCUENTASAET VARCHAR[] :='{28050508,13802704}';--IDEAL UN TABLA OJO!!
...
--SE CONSULTAN LOS REGISTROS QUE SE VAN A TRANSPORTAR PARA TRANSFERENCIAS
	...
	FOR REC_OS IN
		...
		SELECT distinct
			...
			A.CREATION_DATE,
			B.CUENTA,
			A.CUENTA AS CUENTA_PLA,
			CASE WHEN HT.NIT_APOTEOSYS IS NOT NULL THEN HT.NIT_APOTEOSYS ELSE A.NITCLI END AS NITCLI,
			A.VALOR_INGRESO as vlr_ingreso,
			A.PERIODO,
			A.TIPO_DOCUMENTO,
			--A.DOCUMENTO,
			--A.DESCRIPCION,
			TRIM(SUBSTRING(A.DESCRIPCION,STRPOS(A.DESCRIPCION, 'Numero Operacion: ')+18 ,STRPOS(A.DESCRIPCION, 'Tipo Documento: ')-(STRPOS(A.DESCRIPCION, 'Numero Operacion: ')+18)))AS NUMERO_OPERACION,
			TRIM(SUBSTRING(A.DESCRIPCION,STRPOS(A.DESCRIPCION, 'Tipo operacion: ')+15 ,STRPOS(A.DESCRIPCION, 'Numero Operacion: ')-(STRPOS(A.DESCRIPCION, 'Tipo operacion: ')+15)))AS TIPO_OPERACION,
			(CASE
		...
		WHERE
			...
			--AND A.PERIODO='201704'
			AND A.DESCRIPCION LIKE '%Tipo operacion:%'
			and A.NUM_INGRESO in('IA531066')
			--AND COALESCE(A.PROCESADO,'N')='N'
			--AND (A.DESCRIPCION LIKE '%392371%')
		--LIMIT 100
ORDER BY 1
	...
	LOOP
		...
		if(CON.OBTENER_HOMOLOGACION_APOTEOSYS('NOTAAJUSTE', REC_OS.TIPO_DOCUMENTO, REC_OS.CUENTA_PLA,'', 6)='S')then
		...
		MCTYPE.MC_____CODIGO____CD_____B := 'IALG'  ;
		...
		MCTYPE.MC_____CODIGO____CPC____B := CON.OBTENER_HOMOLOGACION_APOTEOSYS('NOTAAJUSTE', REC_OS.TIPO_DOCUMENTO, REC_OS.CUENTA_PLA,'', 1)  ;
		MCTYPE.MC_____CODIGO____CU_____B := CASE WHEN REC_OS.TIPO_OPERACION='AET' THEN 'A1111F21401'
							WHEN REC_OS.TIPO_OPERACION='EXT' THEN 'A1111F22201'
							WHEN REC_OS.TIPO_OPERACION='AGA' THEN 'A1111F21501'
							ELSE '' END;
		--CON.OBTENER_HOMOLOGACION_APOTEOSYS('NOTAAJUSTE', REC_OS.TIPO_DOCUMENTO, _ARRCUENTASAET[2],'', 2)  ;
		...
		IF(REC_OS.VLR_INGRESO<0)THEN
			MCTYPE.MC_____DEBMONLOC_B := REC_OS.VLR_INGRESO::NUMERIC*-1  ;
			MCTYPE.MC_____CREMONLOC_B := 0  ;
		ELSE
			MCTYPE.MC_____DEBMONLOC_B := 0;
			MCTYPE.MC_____CREMONLOC_B := REC_OS.VLR_INGRESO::NUMERIC  ;
		END IF;

		-- IF(REC_OS.VLR_INGRESO<0)THEN
-- 			MCTYPE.MC_____CREMONLOC_B := REC_OS.VLR_INGRESO::NUMERIC*-1  ;
-- 			MCTYPE.MC_____DEBMONLOC_B := 0  ;
-- 		ELSE
-- 			MCTYPE.MC_____CREMONLOC_B := 0;
-- 			MCTYPE.MC_____DEBMONLOC_B := REC_OS.VLR_INGRESO::NUMERIC  ;
-- 		END IF;

		MCTYPE.MC_____INDTIPMOV_B := 4  ;
		MCTYPE.MC_____INDMOVREV_B := 'N'  ;
		MCTYPE.MC_____OBSERVACI_B := REC_OS.numero_operacion;
		...
		MCTYPE.MC_____CODIGO____DS_____B := CON.OBTENER_HOMOLOGACION_APOTEOSYS('NOTAAJUSTE', REC_OS.TIPO_DOCUMENTO, REC_OS.CUENTA_PLA,'', 3);
		MCTYPE.MC_____NUMDOCSOP_B := REC_OS.NUMERO_OPERACION;
		MCTYPE.MC_____NUMEVENC__B := CON.OBTENER_HOMOLOGACION_APOTEOSYS('NOTAAJUSTE', REC_OS.TIPO_DOCUMENTO, REC_OS.CUENTA_PLA,'', 5)::INT;

		if(CON.OBTENER_HOMOLOGACION_APOTEOSYS('NOTAAJUSTE', REC_OS.TIPO_DOCUMENTO, REC_OS.CUENTA_PLA,'', 4)='S')then
			if(REC_OS.tipo_operacion='AGA')then
		...
		if(CON.OBTENER_HOMOLOGACION_APOTEOSYS('NOTAAJUSTE', REC_OS.TIPO_DOCUMENTO, REC_OS.CUENTA_PLA,'', 5)::int=1)then
		...
		MCTYPE.MC_____SECUINTE__DCD____B := CONSEC  ;
		MCTYPE.MC_____SECUINTE__B := CONSEC  ;
		MCTYPE.MC_____CODIGO____CPC____B := CON.OBTENER_HOMOLOGACION_APOTEOSYS('NOTAAJUSTE', REC_OS.TIPO_DOCUMENTO, REC_OS.CUENTA,'', 1)  ;
		--MCTYPE.MC_____CODIGO____CU_____B := CON.OBTENER_HOMOLOGACION_APOTEOSYS('NOTAAJUSTE', REC_OS.TIPO_DOCUMENTO, REC_OS.CUENTA,'', 2)  ;
		...
		IF(SW='S')THEN
			...
			IF CON.SP_VALIDACIONES(MCTYPE, 'LOGISTICA') ='N' THEN
				...
				WHERE MC_____NUMERO____B = SECUENCIA_EXT AND MC_____CODIGO____CONTAB_B = 'FINT'
				 AND MC_____CODIGO____TD_____B = 'ICRN' AND  MC_____CODIGO____CD_____B = 'IALG'  ;
...
ALTER FUNCTION con.interfaz_fintra_logistica_apoteosys_pp_ica()
  OWNER TO postgres;
