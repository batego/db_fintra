-- Function: con.interfaz_automotor_apoteosys()

-- DROP FUNCTION con.interfaz_automotor_apoteosys();

CREATE OR REPLACE FUNCTION con.interfaz_automotor_apoteosys()
  RETURNS text AS
$BODY$
...
DECLARE
 ...
 /************************************************
  *DESCRIPCION: ESTA FUNCION BUSCA TODOS LO NEGOCIOS AUTOMOTOR.
  *AUTOR:=@MMEDINA
...
BEGIN
	...
	FOR NEGOCIO_ IN
		select
			...
			neg.financia_aval,
			neg.negocio_rel,
			replace(substring(neg.f_desem,1,7),'-','')::NUMERIC AS periodo_neg
		from negocios neg
		...
		Where neg.estado_neg = 'T' and uneg.id = '13'
		and procesado_mc = 'N'
		AND neg.cod_neg not in (select negocio_reestructuracion from rel_negocios_reestructuracion)
		and neg.financia_aval = true
		and neg.negocio_rel = ''
		and replace(substring(neg.f_desem,1,7),'-','')::numeric between '201701'::numeric and replace(substring(now(),1,7),'-','')::numeric
	LOOP
-- 		SELECT INTO SECUENCIA_GEN NEXTVAL('CON.INTERFAZ_SECUENCIA_OPER_APOTEOSYS');
		...
		LOOP
			...
			IF(CON.OBTENER_HOMOLOGACION_APOTEOSYS('AUTOMOTOR',INFOITEMS_.tipo_documento, CUENTA_ASIENTO, NEGOCIO_.agencia, 6)='S')THEN
			...
			MCTYPE.MC_____CODIGO____CPC____B := CON.OBTENER_HOMOLOGACION_APOTEOSYS('AUTOMOTOR', INFOITEMS_.tipo_documento, CUENTA_ASIENTO, NEGOCIO_.agencia, 1);
			MCTYPE.MC_____CODIGO____CU_____B := CON.OBTENER_HOMOLOGACION_APOTEOSYS('AUTOMOTOR', INFOITEMS_.tipo_documento, CUENTA_ASIENTO, NEGOCIO_.agencia, 2);
			...
			MCTYPE.MC_____CODIGO____DS_____B := CON.OBTENER_HOMOLOGACION_APOTEOSYS('AUTOMOTOR',INFOITEMS_.tipo_documento, CUENTA_ASIENTO, NEGOCIO_.agencia, 3);
			IF(CON.OBTENER_HOMOLOGACION_APOTEOSYS('AUTOMOTOR', INFOITEMS_.tipo_documento, CUENTA_ASIENTO,NEGOCIO_.agencia, 4)='S')THEN
			...
			IF(CON.OBTENER_HOMOLOGACION_APOTEOSYS('AUTOMOTOR', INFOITEMS_.tipo_documento, CUENTA_ASIENTO, NEGOCIO_.agencia, 5)::INT=1)THEN
		...
		if(SW = 'S')then
			...
			RESPUESTA := con.interfaz_automotor_aval_apoteosys(NEGOCIO_.cod_neg, MCTYPE.MC_____NUMERO____B, MCTYPE.MC_____CODIGO____TD_____B, MCTYPE.MC_____CODIGO____CD_____B,INFOITEMS_.creation_date::date);
			IF(RESPUESTA = 'N')then
				CONTINUE;
			end if;
...
ALTER FUNCTION con.interfaz_automotor_apoteosys()
  OWNER TO postgres;
