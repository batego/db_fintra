-- Function: con.interfaz_micro_nc_cxc_efecty_apoteosys()

-- DROP FUNCTION con.interfaz_micro_nc_cxc_efecty_apoteosys();

CREATE OR REPLACE FUNCTION con.interfaz_micro_nc_cxc_efecty_apoteosys()
  RETURNS text AS
$BODY$
...
BEGIN
	...
	FOR NOTAS_ IN
		...
		where
			...
			--cmc in('SE','SU') and
			ing.periodo BETWEEN '201701' AND replace(substring(current_date,1,7),'-','') and
			ing.nitcli='8301319931'
			and coalesce(ingd.procesado_ica,'N')='N'
		group by
			ing.tipo_documento,
			ing.num_ingreso,
			ing.fecha_ingreso,
		...
		--select con.interfaz_micro_nc_cxc_efecty_apoteosys()
		/**
...
ALTER FUNCTION con.interfaz_micro_nc_cxc_efecty_apoteosys()
  OWNER TO postgres;
