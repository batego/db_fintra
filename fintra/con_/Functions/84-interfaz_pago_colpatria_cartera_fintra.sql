-- Function: con.interfaz_pago_colpatria_cartera_fintra()

-- DROP FUNCTION con.interfaz_pago_colpatria_cartera_fintra();

CREATE OR REPLACE FUNCTION con.interfaz_pago_colpatria_cartera_fintra()
  RETURNS text AS
$BODY$
...
BEGIN
	...
	/**
	SELECT CON.INTERFAZ_PAGO_COLPATRIA_CARTERA_FINTRA()
	...
	UPDATE con.mc_sl_fac_sel set procesado='W'
	--,mc_____IDENTIFIC_TERCER_B=89010201
	where
	MC_____NUMERO____B in('79197') and
	MC_____CODIGO____CD_____B in('CCFR','CCFN') and
	mc_____IDENTIFIC_TERCER_B=890102018
	...
	*/
	...
	LOOP
		...
		_ARRAYNEGOCIOS:=(select array(
			...
			AND FACd.REG_STATUS = ''
			AND id.cuenta IN(FACTURA_RE.CUENTA))::varchar[]);
...
ALTER FUNCTION con.interfaz_pago_colpatria_cartera_fintra()
  OWNER TO postgres;
