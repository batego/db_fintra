-- Function: con.eg_recaudo_xunidad_xingreso(character varying, character varying, character varying)

-- DROP FUNCTION con.eg_recaudo_xunidad_xingreso(character varying, character varying, character varying);

CREATE OR REPLACE FUNCTION con.eg_recaudo_xunidad_xingreso(_periodo character varying, _unidadnego character varying, _numingreso character varying)
...
DECLARE
 ...
 cursor_recaudo CURSOR(_periodo varchar) FOR (SELECT substring(idet.periodo,1,4) as anio,
						...
						AND idet.num_ingreso in (_numIngreso)
...
ALTER FUNCTION con.eg_recaudo_xunidad_xingreso(character varying, character varying, character varying)
  OWNER TO postgres;
