-- Function: tem.dv_reporte_cartera_global_educativo_codeudor()

-- DROP FUNCTION tem.dv_reporte_cartera_global_educativo_codeudor();

CREATE OR REPLACE FUNCTION tem.dv_reporte_cartera_global_educativo_codeudor()
...
$BODY$
	...
	BEGIN
		...
		FOR CarteraGeneral IN
			...
			LEFT JOIN SOLICITUD_PERSONA SPC ON (SPC.NUMERO_SOLICITUD=SA.NUMERO_SOLICITUD AND SPC.TIPO in ('C'))
			...
			WHERE FRA.DSTRCT = 'FINV'
					   ...
					   -- AND NEGOCIOS.COD_NEG = 'FA32209'
					    ...
					    AND NEGOCIO_REL_GPS = ''
					    AND UN.ID in (2,8,31) AND UN.REF_4 !=''
...
ALTER FUNCTION tem.dv_reporte_cartera_global_educativo_codeudor()
  OWNER TO postgres;
