-- Function: tem.interfaz_microcredito_egreson_apoteosys(character varying)

-- DROP FUNCTION tem.interfaz_microcredito_egreson_apoteosys(character varying);

CREATE OR REPLACE FUNCTION tem.interfaz_microcredito_egreson_apoteosys(periodo_ character varying)
  RETURNS text AS
$BODY$
...
BEGIN
	...
	FOR NEGOCIO_ IN
		...
		AND NEG.PERIODO = periodo_ --REPLACE(SUBSTRING(CURRENT_DATE,1,7),'-','')
		AND COD_NEG LIKE 'MC%'
...
ALTER FUNCTION tem.interfaz_microcredito_egreson_apoteosys(character varying)
  OWNER TO postgres;
