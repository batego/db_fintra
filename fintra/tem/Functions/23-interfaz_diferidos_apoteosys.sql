-- Function: tem.interfaz_diferidos_apoteosys(integer, character varying)

-- DROP FUNCTION tem.interfaz_diferidos_apoteosys(integer, character varying);

CREATE OR REPLACE FUNCTION tem.interfaz_diferidos_apoteosys(_idunidadnegocio integer, _periodo character varying)
  RETURNS text AS
$BODY$
...
BEGIN
	...
	FOR NEGOCIO_ IN
		...
		and procesado_dif = 'N'
		AND a.periodo = _periodo ---BETWEEN '201701' AND REPLACE(SUBSTRING(CURRENT_DATE,1,7),'-','')
...
ALTER FUNCTION tem.interfaz_diferidos_apoteosys(integer, character varying)
  OWNER TO postgres;
