-- Function: tem.dv_detallecarteraxsancion(numeric, character varying, character varying)

-- DROP FUNCTION tem.dv_detallecarteraxsancion(numeric, character varying, character varying);

CREATE OR REPLACE FUNCTION tem.dv_detallecarteraxsancion(periodoasignacion numeric, unidadnegocio character varying, negocioref character varying)
				...
				FOR _sanciofa IN (SELECT * FROM con.factura_detalle where documento = CarteraGeneral.documento) LOOP
					...
					if ( VerifyDetails = 'S' ) then
						...
						LOOP
							...
							END IF;
raise notice '_Base: % CarteraGeneral.fecha_vencimiento: % _IxM: % _Tasa: % (now()::date - CarteraGeneral.fecha_vencimiento)::numeric : %',_Base, CarteraGeneral.fecha_vencimiento, _IxM, _Tasa, (now()::date - CarteraGeneral.fecha_vencimiento)::numeric ;
...
ALTER FUNCTION tem.dv_detallecarteraxsancion(numeric, character varying, character varying)
  OWNER TO postgres;
