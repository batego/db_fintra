-- Function: tem.dv_reporte_cartera_global_codeudor(character varying)

-- DROP FUNCTION tem.dv_reporte_cartera_global_codeudor(character varying);

CREATE OR REPLACE FUNCTION tem.dv_reporte_cartera_global_codeudor(unidadnegocio_ character varying)
	...
	BEGIN
		...
		FOR CarteraGeneral IN
			...
			SELECT NEGOCIOS.COD_NEG::VARCHAR AS NEGOCIO,
				...
				CASE WHEN SPC.NOMBRE IS NULL THEN SP.NOMBRE ELSE SPC.NOMBRE END AS NOMBRE_CLIENTE,
				--UPPER(SP.NOMBRE) AS NOMBRE_CLIENTE,
				--UPPER(PRIMER_NOMBRE)::VARCHAR AS PRIMER_NOMBRE,
				CASE WHEN SPC.PRIMER_NOMBRE IS NULL THEN SP.PRIMER_NOMBRE ELSE SPC.PRIMER_NOMBRE END AS PRIMER_NOMBRE,
				...
				CASE WHEN SPC.DIRECCION IS NULL THEN SP.DIRECCION ELSE SPC.DIRECCION END AS DIRECCION,
				--SP.BARRIO::VARCHAR,
				CASE WHEN SPC.BARRIO IS NULL THEN SP.BARRIO ELSE SPC.BARRIO END AS BARRIO,
				--SP.CIUDAD::VARCHAR,
				CASE WHEN SPC.CIUDAD IS NULL THEN SP.CIUDAD ELSE SPC.CIUDAD END AS CIUDAD,
				--COALESCE(CLI.TELEFONO ||' - '|| CLI.TELCONTACTO,'NR')::VARCHAR AS TEL,
				CASE WHEN SPC.CELULAR IS NULL THEN (CLI.TELEFONO ||' - '|| CLI.TELCONTACTO) ELSE SPC.CELULAR END AS TEL,
				--UPPER(SP.EMAIL)::VARCHAR AS CORREO_ELECTRONICO,
				CASE WHEN SPC.EMAIL IS NULL THEN SP.EMAIL ELSE SPC.EMAIL END AS EMAIL,
			...
			LEFT JOIN SOLICITUD_PERSONA SPC ON (SPC.NUMERO_SOLICITUD=SA.NUMERO_SOLICITUD AND SPC.TIPO ='C')
			...
			WHERE FRA.DSTRCT = 'FINV'
					    ...
					    AND NEGOCIO_REL_GPS = ''
					    AND UN.ID= unidadNegocio_ AND UN.REF_4 !=''
					   --AND NEGOCIOS.PERIODO = '201701'


			GROUP BY NEGOCIOS.COD_NEG, FRA.NIT, SP.NOMBRE, SP.PRIMER_NOMBRE, CLI.NOMCLI, UN.DESCRIPCION, CO.NOMBRE, SP.DIRECCION, SP.BARRIO, SP.CIUDAD, CLI.TELEFONO, CLI.TELCONTACTO, SP.EMAIL, FINANCIA_AVAL,
			SPC.NOMBRE ,SPC.PRIMER_NOMBRE, SPC.DIRECCION, SPC.BARRIO, SPC.CIUDAD, SPC.CELULAR, SPC.EMAIL
...
ALTER FUNCTION tem.dv_reporte_cartera_global_codeudor(character varying)
  OWNER TO postgres;
