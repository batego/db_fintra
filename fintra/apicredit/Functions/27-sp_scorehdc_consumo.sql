-- Function: apicredit.sp_scorehdc_consumo(integer)

-- DROP FUNCTION apicredit.sp_scorehdc_consumo(integer);

CREATE OR REPLACE FUNCTION apicredit.sp_scorehdc_consumo(_numero_solicitud integer)
  RETURNS text AS
$BODY$
...
DECLARE
	...
	startClock TIMESTAMP:=clock_timestamp();
...
BEGIN
	...
	raise notice 'puntaje_maximo_buro: %', RsPuntajes.puntaje_maximo_buro;
	raise notice 'minimo_buro: %', RsPuntajes.minimo_buro;
	raise notice 'maximo_buro: %', RsPuntajes.maximo_buro;
	raise notice 'cutoff_buro: %', RsPuntajes.cutoff_buro;
	...
	if ( RsPuntajes is not null ) then
		...
		raise notice 'RsPresolicitudes-IF: %', ( RsPresolicitudes is not null );
			...
			if ( ConfirmarHDC > 0 ) then
				...
				LOOP
					...
					if ( vble_mercado_buro.id = 21 ) then
					 ...
					 raise notice 'vble_mercado_buro.id %  tiempo : % ',vble_mercado_buro.id,(clock_timestamp()-startClock)::interval ;
					 startClock:=clock_timestamp();
...
ALTER FUNCTION apicredit.sp_scorehdc_consumo(integer)
  OWNER TO postgres;
