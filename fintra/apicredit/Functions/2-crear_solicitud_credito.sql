-- Function: apicredit.crear_solicitud_credito(integer, integer, character varying)

-- DROP FUNCTION apicredit.crear_solicitud_credito(integer, integer, character varying);

CREATE OR REPLACE FUNCTION apicredit.crear_solicitud_credito(_numero_solicitud integer, _und_negocio integer, _recoje_firmas character varying)
  RETURNS text AS
$BODY$
...
BEGIN
	...
	_RESULT:= apicredit.eg_generar_negocio_credito(_UND_NEGOCIO,_NUMERO_SOLICITUD, _DATOSPRESOSLICITUD.CREATION_USER, false, _ENTIDAD, 'N', _RECOJE_FIRMAS, '{}'::text[]);

	RETURN _RESULT;

END $BODY$
  LANGUAGE plpgsql VOLATILE;
ALTER FUNCTION apicredit.crear_solicitud_credito(integer, integer, character varying)
  OWNER TO postgres;
