-- Function: sp_det_facturas_indemnizar_fianza_micro(character varying, character varying, integer, character varying, integer, character varying, boolean, boolean, boolean)

-- DROP FUNCTION sp_det_facturas_indemnizar_fianza_micro(character varying, character varying, integer, character varying, integer, character varying, boolean, boolean, boolean);

CREATE OR REPLACE FUNCTION sp_det_facturas_indemnizar_fianza_micro(_negocio character varying, periodo_corte character varying, _linea_negocio integer, _empresa_fianza character varying, _mora integer, filtro character varying, acelerar_pagare boolean, _gac boolean, _ixm boolean)
	...
	-- AND (sp_fecha_corte_foto(substring(periodo_lote,1,4),substring(periodo_lote,5,6)::int)::date-foto.fecha_vencimiento::date)  > '|| _mora ||
       ...
       FOR result_var IN EXECUTE SQL LOOP
        ...
        INSERT INTO tem.facturas_indemnizar_fianza_micro
         ...
         result_var.ref_4,result_var.cartera_en,result_var.estado, result_var.valor_saldo_cm,result_var.valor_saldo_seguro,result_var.valor_abono);
       ...
       IF(acelerar_pagare)THEN
	       ...
	       LOOP
			--BUSCA LAS FACTURAS FUTURAS DEL NEGOCIO.
				...
				foto.valor_factura::numeric as valor_factura,
				fdet.valor_unitario as valor_saldo_capital,
				0.00::numeric as valor_desistir,
				'0099-01-01'::date as fecha_indemnizacion,
				 0::numeric as valor_saldo_mi,
				 0::numeric as valor_saldo_ca,
				...
				''::varchar as cartera_en,
                                (CASE WHEN ((sp_fecha_corte_foto(substring(periodo_corte,1,4),substring(periodo_corte,5,6)::int)::date-foto.fecha_vencimiento::date)::int>0) THEN 'VENCIDO' ELSE 'FUTURA' END)::varchar as estado,
                                 0::numeric as valor_saldo_cm,
                                0.00::NUMERIC AS valor_saldo_seguro,
				foto.valor_abono::NUMERIC
			FROM con.foto_cartera as foto
			INNER JOIN con.factura_detalle fdet on (foto.documento=fdet.documento and fdet.descripcion='CAPITAL')
	...
	--arma la consulta para realizar los calculos de sanciones y de mas
		...
		IF (listaFacturas.dias_mora > 0) THEN
			...
			IF(listaFacturas.esquema_old='S')THEN
				...
				/* *************************************************************
				* Calculamos el saldo para el seguro  y capital por cuota   *
				****************************************************************/
				restoAplicacionSeguro:=listaFacturas.valor_saldo_seguro - listaFacturas.valor_abono;
				raise notice 'listaFacturas.valor_saldo_capital : % listaFacturas.valor_saldo_seguro: % listaFacturas.valor_abono: % restoAplicacionSeguro : %',listaFacturas.valor_saldo_capital,listaFacturas.valor_saldo_seguro,listaFacturas.valor_abono,restoAplicacionSeguro;
				IF(restoAplicacionSeguro >= 0)THEN
				     listaFacturas.valor_saldo_seguro:=restoAplicacionSeguro;
				ELSE
				     listaFacturas.valor_saldo_seguro:=0;
				     restoAplicacionSeguro:=(restoAplicacionSeguro * -1);
				     listaFacturas.valor_saldo_capital:=listaFacturas.valor_saldo_capital - restoAplicacionSeguro;
				 END IF;
			...
			ELSE
				...
				FROM  eg_detalle_saldo_facturas_mc_foto(listaFacturas.negocio::VARCHAR,listaFacturas.cuota::INTEGER,periodo_corte::varchar);

				listaFacturas.valor_saldo_capital:=_detalleSaldoFacturas.saldo_capital;
				listaFacturas.valor_saldo_seguro:=_detalleSaldoFacturas.saldo_seguro;
			...
			**********************************************************/
			sumaConceptos=listaFacturas.valor_saldo_capital+listaFacturas.valor_saldo_seguro+listaFacturas.valor_saldo_cm+listaFacturas.valor_saldo_mi + listaFacturas.valor_saldo_ca ;

			--validamos si se dbe cobrar el ixmora
			IF(_ixm)THEN
			...
			*************************************************/
			...
			IF(resta between 27  and 34)THEN --esta condicion solo me sirve para saber cual es al cuota
			    ...
			    IF(diasInteresCorriente > 30) THEN diasInteresCorriente:=30; END IF;
			      ...
			      saldoMI:=ROUND((filaLiquidador.interes/30)*(diasInteresCorriente));
                             --saldoMI :=nuevoInteres;
			END IF;

			listaFacturas.valor_saldo_capital:=listaFacturas.valor_saldo_capital ; ---filaLiquidador.seguro
			listaFacturas.valor_saldo_seguro:=0.00;
			listaFacturas.valor_saldo_mi:=saldoMI;
			listaFacturas.valor_saldo_ca=filaLiquidador.cat;
			listaFacturas.valor_saldo_cm:=0.00;
			listaFacturas.total_saldo= listaFacturas.valor_saldo_capital + listaFacturas.valor_saldo_seguro + listaFacturas.valor_saldo_mi + listaFacturas.valor_saldo_ca;
...
ALTER FUNCTION sp_det_facturas_indemnizar_fianza_micro(character varying, character varying, integer, character varying, integer, character varying, boolean, boolean, boolean)
  OWNER TO postgres;
