-- Function: corregir_fechas_contables_cxps_pf()

-- DROP FUNCTION corregir_fechas_contables_cxps_pf();

CREATE OR REPLACE FUNCTION corregir_fechas_contables_cxps_pf()
  RETURNS text AS
$BODY$DECLARE
    _group RECORD;
    _count INTEGER;
...
SELECT * FROM (
	...
	FROM (
		SELECT (SELECT MAX(f_facturado_cliente)
					FROM app_accord
					WHERE factura_formula_provintegral=c.documento
		...
		WHERE SUBSTR(documento,1,2) IN ('PF') AND proveedor='9000742958' AND tipo_documento IN ('FAP')  AND numdoc=documento AND tipodoc='FAP' AND com.grupo_transaccion=c.transaccion
...
ALTER FUNCTION corregir_fechas_contables_cxps_pf()
  OWNER TO postgres;
