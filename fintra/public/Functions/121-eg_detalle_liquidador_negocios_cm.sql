-- Function: eg_detalle_liquidador_negocios_cm()

-- DROP FUNCTION eg_detalle_liquidador_negocios_cm();

CREATE OR REPLACE FUNCTION eg_detalle_liquidador_negocios_cm()
...
BEGIN
	...
	FOR record_liquidador IN (SELECT
				...
				INNER JOIN ing_fenalco ing ON (dna.cod_neg=ing.codneg and ing.fecha_doc::date=dna.fecha::date AND ing.tipodoc  like 'CM%')
				...
				AND neg.fecha_negocio::date  between '2016-01-01' AND substring(now(),1,4)||'-12-31'
				AND dna.reg_status=''
				--and dna.cod_neg='FA33473'
...
ALTER FUNCTION eg_detalle_liquidador_negocios_cm()
  OWNER TO postgres;
