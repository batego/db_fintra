-- Function: anulardiferidos(character varying, text[])

-- DROP FUNCTION anulardiferidos(character varying, text[]);

CREATE OR REPLACE FUNCTION anulardiferidos(_negocio character varying, _diferidos text[])
  RETURNS text AS
$BODY$
DECLARE
		respuesta        CHARACTER VARYING = '';
		registro         CHARACTER VARYING;
		diferidos        CHARACTER VARYING [];
		contador         INT = 0;
...
BEGIN
		...
		RAISE NOTICE 'DIFERIDOS %', diferidos;

		IF contador = array_upper(_diferidos, 1)
		THEN
				RETURN 'Diferidos anulados';
		ELSE
				RETURN respuesta;
		END IF;
END;
...
ALTER FUNCTION anulardiferidos(character varying, text[])
  OWNER TO postgres;
