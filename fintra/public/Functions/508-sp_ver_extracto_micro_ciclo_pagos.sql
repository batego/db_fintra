-- Function: sp_ver_extracto_micro_ciclo_pagos(numeric, character varying, numeric, date, character varying, character varying)

-- DROP FUNCTION sp_ver_extracto_micro_ciclo_pagos(numeric, character varying, numeric, date, character varying, character varying);

CREATE OR REPLACE FUNCTION sp_ver_extracto_micro_ciclo_pagos(und_negocio numeric, periodo_corriente character varying, nciclo numeric, fecha_corte date, accion character varying, usuario character varying)
	...
	FOR CarteraxCliente IN EXECUTE SQL LOOP
		...
		if(accion = 'Generar') then
			...
			INSERT INTO recibo_oficial_pago (cod_rop, id_unidad_negocio, periodo_rop, id_ciclo, vencimiento_rop, negocio, cuotas_vencidas, cuotas_pendientes, dias_vencidos, subtotal, total_sanciones, total_descuentos, total, total_abonos, creation_date, creation_user, last_update, user_update, observacion)
			  VALUES('',1,periodo_corriente,Ciclo.id,CarteraxCliente.vencimiento_rop,CarteraxCliente.negasoc,'0','0/0','0',0,0,0,0,0,now(),usuario,now(),usuario,'')
		...
		IF FOUND THEN
			...
			FechaCalculoInteres = FchLastPay.fecha_consignacion::date;
		--ELSE
		...
		FOR CarteraxCuota IN
			...
			SELECT negocio, cedula, prefijo, cuota, fecha_factura, fecha_vencimiento, sum(valor_saldo) as valor_saldo, (fecha_corte::date-fecha_vencimiento::DATE) AS dias_vencidos FROM (
				...
				SELECT
					...
					case when substring(f.documento,1,2) in ('MC','TD','R0') then f.fecha_factura else (select fecha_factura from con.factura mf where mf.documento = (select numero_remesa from con.factura_detalle where documento = f.documento limit 1)) end as fecha_factura,
					case when substring(f.documento,1,2) in ('MC','TD','R0') then f.fecha_vencimiento else (select fecha_vencimiento from con.factura mf where mf.documento = (select numero_remesa from con.factura_detalle where documento = f.documento limit 1)) end as fecha_vencimiento,
					f.valor_saldo
				FROM con.foto_ciclo_pagos f
				WHERE f.reg_status = ''
					and f.dstrct = 'FINV'
					and f.tipo_documento = 'FAC'
					and f.valor_saldo > 0
					and substring(f.documento,1,2) in ('MC','TD','MI','CA','R0')
					and f.negasoc = CarteraxCliente.negasoc
		...
		LOOP
			...
			_Base = 0;
			_GaC = 0;
			IF (CarteraxCuota.prefijo IN ('MC','TD','R0')) THEN
				_SumCapital = _SumCapital + CarteraxCuota.valor_saldo;
			ELSIF (CarteraxCuota.prefijo in ('MI','TS')) THEN
		...
		if ( _TotalCuotasPendientes.maxdia <= 0 ) then
			...
			MsgPagueseAntes = (select max(fecha_vencimiento) from con.foto_ciclo_pagos where negasoc = CarteraxCliente.negasoc and valor_saldo > 0 and fecha_vencimiento <= Ciclo.fecha_fin and periodo_lote = periodo_corriente /*and num_ciclo = ciclo and replace(substring(fecha_vencimiento,1,7),'-','')::numeric = periodo_corriente*/);
...
ALTER FUNCTION sp_ver_extracto_micro_ciclo_pagos(numeric, character varying, numeric, date, character varying, character varying)
  OWNER TO postgres;
