-- Function: corregir_fechas_contables_ncs_ap()

-- DROP FUNCTION corregir_fechas_contables_ncs_ap();

CREATE OR REPLACE FUNCTION corregir_fechas_contables_ncs_ap()
  RETURNS text AS
$BODY$DECLARE
    _group RECORD;
    _count INTEGER;
...
SELECT * FROM (
	...
	FROM (
		...
		WHERE SUBSTR(documento,1,2) IN ('AP') AND proveedor='8305137738' AND tipo_documento IN ('NC')  AND numdoc=documento AND documento_relacionado=factura_app

	) ali
	ORDER BY numdoc
) correccion_contable_ncs_ap
...
ALTER FUNCTION corregir_fechas_contables_ncs_ap()
  OWNER TO postgres;
