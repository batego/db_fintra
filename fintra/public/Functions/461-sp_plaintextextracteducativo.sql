-- Function: sp_plaintextextracteducativo(numeric, numeric, date)

-- DROP FUNCTION sp_plaintextextracteducativo(numeric, numeric, date);

CREATE OR REPLACE FUNCTION sp_plaintextextracteducativo(unidadnegocio numeric, periodoasignacion numeric, fue_generado date)
...
BEGIN
	...
	FOR CarteraExtracto IN
		...
		SELECT
			...
			''::varchar as direccion, --DIRECCION --rop.direccion::varchar,
			''::varchar as departamento, --DEPARTAMENTO
			''::varchar as ciudad, --CIUDAD
			''::varchar as barrio, --BARRIO
			'EDUCATIVO'::varchar as linea_producto,
			...
			0::numeric as ktotal
			--select *
		FROM recibo_oficial_pago rop where id_unidad_negocio = unidadnegocio and periodo_rop = periodoasignacion::varchar LOOP --and negocio = ''FA06337'
			...
			END LOOP;
				...
				--select into SolicitudPersona * from solicitud_persona where numero_solicitud = (select numero_solicitud from solicitud_aval where cod_neg = (select max(cod_neg) from negocios where cod_cli = CarteraExtracto.cedula and substring(cod_neg,1,2) != 'NG')) and tipo = 'S';
	...
	END LOOP;
		...
		where rop.id = dr.id_rop and id_unidad_negocio = 2 and periodo_rop = '201406'
		group by
...
ALTER FUNCTION sp_plaintextextracteducativo(numeric, numeric, date)
  OWNER TO postgres;
