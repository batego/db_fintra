-- Function: sp_ver_extracto_micro(numeric, date, character varying, date, character varying, character varying)

-- DROP FUNCTION sp_ver_extracto_micro(numeric, date, character varying, date, character varying, character varying);

CREATE OR REPLACE FUNCTION sp_ver_extracto_micro(und_negocio numeric, fecha_hoy date, periodo_corriente character varying, vcto_rop date, accion character varying, usuario character varying)
...
BEGIN
	delete from tem.extracto_temp;
	  ...
	  SQL = 'select
		...
		WHERE f.reg_status = ''''
			...
			--and f.negasoc=''MC00767''
			and replace(substring(f.fecha_vencimiento,1,7),''-'','''')::numeric <='|| periodo_corriente||'::numeric
			and f.periodo_lote ='|| periodo_corriente||'::numeric GROUP BY id_convenio, negasoc,f.nit';
	...
	LOOP
			...
			SELECT INTO _TotalCuotasVencidas count(0) as CtasVencidas from con.foto_cartera where reg_status = '' and dstrct = 'FINV' and tipo_documento = 'FAC' and negasoc = CarteraxCliente.negasoc and id_convenio = CarteraxCliente.id_convenio and valor_saldo > 0 and periodo_lote = periodo_corriente and replace(substring(fecha_vencimiento,1,7),'-','')::numeric < periodo_corriente and substring(documento,1,2) != 'CP';
			...
			SELECT INTO _TotalCuotasFaltantes count(0) as CtasFaltantes from con.foto_cartera where reg_status = '' and dstrct = 'FINV' and tipo_documento = 'FAC' and negasoc = CarteraxCliente.negasoc and id_convenio = CarteraxCliente.id_convenio and valor_saldo > 0 and periodo_lote = periodo_corriente and replace(substring(fecha_vencimiento,1,7),'-','')::numeric > periodo_corriente and substring(documento,1,2) != 'CP' ;
			...
			if(accion = 'Generar') then
				...
				INSERT INTO recibo_oficial_pago (cod_rop, id_unidad_negocio, periodo_rop, vencimiento_rop, negocio, cuotas_vencidas, cuotas_pendientes, dias_vencidos, subtotal, total_sanciones, total_descuentos, total, total_abonos, creation_date, creation_user, last_update, user_update, observacion)
				  VALUES('',1,periodo_corriente,vcto_rop,CarteraxCliente.negasoc,'0','0/0','0',0,0,0,0,0,now(),usuario,now(),usuario,'')
...
ALTER FUNCTION sp_ver_extracto_micro(numeric, date, character varying, date, character varying, character varying)
  OWNER TO postgres;
