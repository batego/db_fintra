-- Function: sp_exportar_extracto_fenalco(numeric, numeric, date)

-- DROP FUNCTION sp_exportar_extracto_fenalco(numeric, numeric, date);

CREATE OR REPLACE FUNCTION sp_exportar_extracto_fenalco(unidadnegocio numeric, periodoasignacion numeric, fue_generado date)
...
BEGIN
	...
	FOR CarteraExtracto IN
		...
		FROM recibo_oficial_pago rop where id_unidad_negocio = unidadnegocio and periodo_rop = periodoasignacion::varchar LOOP
			...
			END LOOP;
				...
				CarteraExtracto.ktotal = CarteraExtracto.ksubtotalneto - (ddscto_interes_cte + ddscto_interes_xmora + ddscto_gastos_cobranza);
...
ALTER FUNCTION sp_exportar_extracto_fenalco(numeric, numeric, date)
  OWNER TO postgres;
