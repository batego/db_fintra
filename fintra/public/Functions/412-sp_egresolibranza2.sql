-- Function: sp_egresolibranza2(character varying, character varying, character varying)

-- DROP FUNCTION sp_egresolibranza2(character varying, character varying, character varying);

CREATE OR REPLACE FUNCTION sp_egresolibranza2(_cxpcheque character varying, _bancotransf character varying, _usuario character varying)
  RETURNS text AS
$BODY$
...
BEGIN
	...
	NoEgreso = InfoCxP.cheque; --BORRAR
	RAISE NOTICE 'EGRESO No: %', NoEgreso;
	_respuesta = NoEgreso;

	IF FOUND THEN

		--SP_estaciones -> Validaciones de las cuentas
...
ALTER FUNCTION sp_egresolibranza2(character varying, character varying, character varying)
  OWNER TO postgres;
