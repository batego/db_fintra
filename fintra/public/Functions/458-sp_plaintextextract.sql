-- Function: sp_plaintextextract(numeric, numeric, date)

-- DROP FUNCTION sp_plaintextextract(numeric, numeric, date);

CREATE OR REPLACE FUNCTION sp_plaintextextract(unidadnegocio numeric, periodoasignacion numeric, fue_generado date)
...
BEGIN
	...
	FOR CarteraExtracto IN
		...
		SELECT
			...
			fue_generado::varchar as generado_el, --'2014-10-01'::varchar as generado_el,
		...
		FROM recibo_oficial_pago rop where id_unidad_negocio = unidadnegocio and periodo_rop = periodoasignacion LOOP
			...
			END LOOP;
				...
				CarteraExtracto.ktotal = CarteraExtracto.ksubtotal - (ddscto_capital + ddscto_interes_cte + ddscto_interes_xmora + ddscto_gastos_cobranza);
...
ALTER FUNCTION sp_plaintextextract(numeric, numeric, date)
  OWNER TO postgres;
