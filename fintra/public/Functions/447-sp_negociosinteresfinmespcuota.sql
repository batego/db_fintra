-- Function: sp_negociosinteresfinmespcuota()

-- DROP FUNCTION sp_negociosinteresfinmespcuota();

CREATE OR REPLACE FUNCTION sp_negociosinteresfinmespcuota()
...
BEGIN
	...
	FOR FacturasCapital IN
		...
		c.cuenta_interes, n.dist, f.documento, f.cmc, now()::date as fecha_doc, n.cod_neg, n.fecha_liquidacion as fecha_ant, d.saldo_inicial, n.tasa, d.fch_interes_causado
		,c.tipo
		from negocios n
		...
		inner join con.factura f on (f.negasoc=n.cod_neg and fecha_vencimiento=d.fecha and f.descripcion!= c.prefijo_cxc_interes and f.descripcion!=c.prefijo_cxc_cat and f.descripcion!=''
		      and substring(f.documento,length(f.documento)-1, 2) !='00')
		inner join (select min(fecha) as fecha, cod_neg from documentos_neg_aceptado  group by cod_neg) m  on (m.cod_neg=d.cod_neg and m.fecha=d.fecha)
		...
		and d.fecha>now()::date and n.fecha_liquidacion<now()::date
...
ALTER FUNCTION sp_negociosinteresfinmespcuota()
  OWNER TO postgres;
