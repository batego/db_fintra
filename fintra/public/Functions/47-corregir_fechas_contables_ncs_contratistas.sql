-- Function: corregir_fechas_contables_ncs_contratistas()

-- DROP FUNCTION corregir_fechas_contables_ncs_contratistas();

CREATE OR REPLACE FUNCTION corregir_fechas_contables_ncs_contratistas()
  RETURNS text AS
$BODY$DECLARE
    _group RECORD;
    _count INTEGER;
...
SELECT * FROM (
	...
	FROM (
		SELECT (SELECT MAX(f_facturado_cliente)
					FROM app_accord
					WHERE factura_contratista=c.documento_relacionado
					GROUP BY factura_contratista) AS fec_open
		...
		WHERE SUBSTR(documento,1,3) IN ('CC0') AND tipo_documento IN ('NC')  AND numdoc=documento AND tipodoc='NC'
	) ali
) correccion_contable_ncs_contratistas
...
ALTER FUNCTION corregir_fechas_contables_ncs_contratistas()
  OWNER TO postgres;
