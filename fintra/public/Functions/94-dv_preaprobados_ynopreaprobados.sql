-- Function: dv_preaprobados_ynopreaprobados()

-- DROP FUNCTION dv_preaprobados_ynopreaprobados();

CREATE OR REPLACE FUNCTION dv_preaprobados_ynopreaprobados()
...
DECLARE
 ...
 cursor_negocios CURSOR(_periodo integer) FOR (SELECT
								      ...
								      round(sum(valor_abono)*100/sum(valor_factura),2) as porcetaje_valor,
								      ...
								      0::numeric as porcentaje_cuotas,
								      (select max(replace(substring(fecha_ultimo_pago,1,7),'-','')) from con.factura where negasoc =neg.cod_neg) as periodo_ultimo_pago,
								      dv_cuota_mora_maxima(fac.negasoc) as cuota_mora_maxima,
								      (select max(replace(substring(fecha,1,7),'-','')) from documentos_neg_aceptado where cod_neg =neg.cod_neg) as periodo_ultima_cuota,
								      eg_altura_mora_periodo(fac.negasoc,_periodo,7,0) as altura_mora_ultima_cuota
								...
								inner join (
									...
									inner join (
											...
											and neg.negocio_rel_seguro='' and neg.negocio_rel_gps='' and neg.id_convenio in (select id_convenio from rel_unidadnegocio_convenios
																					 where id_unid_negocio in (select id from unidad_negocio where id = 1))
											and neg.creation_date::date >='2016-01-01'
											group by  cod_cli
		...
		loop
			...
			--IF(recordNegocios.altura_mora_actual = '1- CORRIENTE' )	THEN
				...
				--0.)calculamos el numero de cuotas.
					...
					--*.)PORCENTAJE CUOTAS
					recordNegocios.porcentaje_cuotas:= recordNegocios.cuotas_pagadas*100/recordNegocios.cuotas;
				...
				if not (recordNegocios.porcetaje_valor = 100.00 and recordNegocios.dias_ultimmo_pago <=180 )then
					...
					if not (recordNegocios.altura_mora::integer <= 30)then
				...
				--2.)Validamos los negocios vigentes..
				 ...
				 if not (recordNegocios.dias_ultimmo_pago <=180 AND
						(
...
ALTER FUNCTION dv_preaprobados_ynopreaprobados()
  OWNER TO postgres;
