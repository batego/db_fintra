-- Function: eg_movimiento_tsp_paso1(integer, character varying, character varying, character varying)

-- DROP FUNCTION eg_movimiento_tsp_paso1(integer, character varying, character varying, character varying);

CREATE OR REPLACE FUNCTION eg_movimiento_tsp_paso1(_filtro integer, _periodoi character varying, _periodof character varying, _tipoanticipo character varying)
...
_arrCuentasATE varchar[] := '{13802701,23050301,I010030024153}';
_arrCuentasAGA varchar[] := '{13802701,23050303,I010030024154}';
_arrCuentasEXT varchar[] := '{13802601,I010020024163,23050201}';
_iterador INTEGER:=1;
...
BEGIN
	...
	--validamos el paso 1 si es para AET  o AGA
	...
	ELSIF(_tipoanticipo='AGA') THEN
		filtro2:=' AND  a.cuenta in ('''||_arrCuentasAGA[1]||''','''||_arrCuentasAGA[2]||''','''||_arrCuentasAGA[3]||''')';
	ELSIF(_tipoanticipo='EXT')THEN
		filtro2:=' AND  a.cuenta in ('''||_arrCuentasEXT[1]||''','''||_arrCuentasEXT[2]||''','''||_arrCuentasEXT[3]||''')';
	END IF;
...
ALTER FUNCTION eg_movimiento_tsp_paso1(integer, character varying, character varying, character varying)
  OWNER TO postgres;
