-- Function: reporteprovisionfiducia()

-- DROP FUNCTION reporteprovisionfiducia();

CREATE OR REPLACE FUNCTION reporteprovisionfiducia()
  RETURNS text AS
$BODY$
...
BEGIN
	...
	TRUNCATE TABLE tem.temfacturas;

	FOR FacturasMS IN

		SELECT
		fra.ref1, cmc, nit, codcli, (select nomcli from cliente where codcli = fra.codcli) as name_cliente,  documento, clasificacion1, fecha_vencimiento, now()::date, REPLACE(substring(fecha_vencimiento,1,7),'-','') as periodo, fra.valor_factura, fra.valor_abono, fra.valor_saldo,
		...
		sql_insert = 'insert into tem.temfacturas (ref1, nit, codcli, cliente, cmc, documento, fiducia, fecha_vencimiento, "'||PeriodoTable||'") values('||''''||FacturasMS.ref1||''''||', '||''''||FacturasMS.nit||''''||', '||''''||FacturasMS.codcli||''''||', '||''''||FacturasMS.name_cliente||''''||', '||''''||FacturasMS.cmc||''''||', '||''''||FacturasMS.documento||''''||', '||''''||FacturasMS.clasificacion1||''''||', '||''''||FacturasMS.fecha_vencimiento||''''||', '||FacturasMS.valor_factura||')';
		EXECUTE sql_insert;
...
ALTER FUNCTION reporteprovisionfiducia()
  OWNER TO postgres;
