-- Function: corregir_fechas_contables_cxps_pr()

-- DROP FUNCTION corregir_fechas_contables_cxps_pr();

CREATE OR REPLACE FUNCTION corregir_fechas_contables_cxps_pr()
  RETURNS text AS
$BODY$DECLARE
    _group RECORD;
    _count INTEGER;
...
SELECT * FROM (
	...
	FROM (
		...
		WHERE SUBSTR(documento,1,2) IN ('PR') AND proveedor='9000742958' AND tipo_documento IN ('FAP')  AND numdoc=documento AND documento=factura_pro
...
ALTER FUNCTION corregir_fechas_contables_cxps_pr()
  OWNER TO postgres;
