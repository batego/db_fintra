-- Function: sp_extractoconsumo(numeric, date, character varying, date, date)

-- DROP FUNCTION sp_extractoconsumo(numeric, date, character varying, date, date);

CREATE OR REPLACE FUNCTION sp_extractoconsumo(unidadneg numeric, fecha_hoy date, periodo_corriente character varying, vcto_rop date, pagar_antesde date)
  RETURNS text AS
$BODY$
...
BEGIN
	...
	FOR CarteraxCliente IN
		...
		WHERE f.reg_status = ''
			...
			and f.id_convenio in (select id_convenio from rel_unidadnegocio_convenios where id_unid_negocio = UnidadNeg) --(18) --select * from convenios order by id_convenio = 18/32
			and f.valor_saldo > 0
	...
	LOOP
			...
			END LOOP; --loop del negocio principal
			--
			...
			FOR _DetalleRopCondonar IN select id_conceptos_recaudo, (select categoria from conceptos_recaudo where id = dr.id_conceptos_recaudo) as categoria, (select descripcion from conceptos_recaudo where id = dr.id_conceptos_recaudo) as descripcion, sum(dr.valor_concepto) as valor_concepto, min(fecha_vencimiento_padre) from detalle_rop dr where id_rop = _Current_codrop group by id_conceptos_recaudo, categoria LOOP

				raise notice 'negasoc: %, id_conceptos_recaudo: %, categoria: %, descripcion: %, min_dias_ven: %, periodo_corriente: %, valor_concepto: %', CarteraxCliente.negasoc, _DetalleRopCondonar.id_conceptos_recaudo, _DetalleRopCondonar.categoria, _DetalleRopCondonar.descripcion, CarteraxCliente.min_dias_ven, periodo_corriente, _DetalleRopCondonar.valor_concepto;
				SalidaText = 'SELECT * FROM sanciones_condonaciones WHERE id_tipo_acto = 2 AND aplicado_a = '||_DetalleRopCondonar.id_conceptos_recaudo || ' AND ' || CarteraxCliente.min_dias_ven || ' BETWEEN dias_rango_ini AND dias_rango_fin AND periodo = ' || periodo_corriente || ' and id_unidad_negocio = UnidadNeg';
				raise notice 'Es: %', SalidaText;
			...
			*/
			...
			SELECT INTO ClienteRec nomcli,direccion,ciudad FROM cliente WHERE nit = CedulaExtemporanea; --CarteraxCuota.cedula;
			...
			_SumIxM = 0;
			_SumGaC = 0;
			FOR NegocioAvales IN select * from negocios where negocio_rel = CarteraxCliente.negasoc LOOP
				...
				IF FOUND THEN
						...
						FOR CarteraxCuotaAval IN
							...
							SELECT negocio, cedula, prefijo, cuota, fecha_factura, fecha_vencimiento, sum(valor_saldo) as valor_saldo, (fecha_hoy-fecha_vencimiento::DATE) AS dias_vencidos FROM (
								...
								WHERE   f.documento = fr.documento
									...
									and fr.tipo_documento in ('FAC','NDC')
									and f.valor_saldo > 0
									and f.negasoc = CarteraxCabeceraCta.negocio --NegocioAvales.cod_neg
									and f.num_doc_fen = CarteraxCabeceraCta.cuota
									and substring(f.documento,1,2) not in ('CP','FF','DF')
							...
							if ( VerifyDetails = 'S' ) then
								...
								if found then
									--Sanciones
									FOR _Sancion IN SELECT * FROM sanciones_condonaciones WHERE id_tipo_acto = 1 AND id_conceptos_recaudo = _ConceptRec.id AND CarteraxCuotaAval.dias_vencidos BETWEEN dias_rango_ini AND dias_rango_fin AND periodo = periodo_corriente LOOP
					...
					--CONDONACIONES
					_SumCxCta = 0;
					FOR _DetalleRopCondonar IN select id_conceptos_recaudo, (select categoria from conceptos_recaudo where id = dr.id_conceptos_recaudo) as categoria, (select descripcion from conceptos_recaudo where id = dr.id_conceptos_recaudo) as descripcion, sum(dr.valor_concepto) as valor_concepto, min(fecha_vencimiento_padre) from detalle_rop dr where id_rop = _Current_codrop and negocio = NegocioAvales.cod_neg group by id_conceptos_recaudo, categoria LOOP

						raise notice 'negasoc: %, id_conceptos_recaudo: %, categoria: %, descripcion: %, min_dias_ven: %, periodo_corriente: %, valor_concepto: %', NegocioAvales.cod_neg, _DetalleRopCondonar.id_conceptos_recaudo, _DetalleRopCondonar.categoria, _DetalleRopCondonar.descripcion, CarteraxCliente.min_dias_ven, periodo_corriente, _DetalleRopCondonar.valor_concepto;
						SalidaText = 'SELECT * FROM sanciones_condonaciones WHERE id_tipo_acto = 2 AND aplicado_a = '||_DetalleRopCondonar.id_conceptos_recaudo || ' AND ' || CarteraxCliente.min_dias_ven || ' BETWEEN dias_rango_ini AND dias_rango_fin AND periodo = ' || periodo_corriente || ' and id_unidad_negocio = UnidadNeg';
						raise notice 'Es: %', SalidaText;
...
ALTER FUNCTION sp_extractoconsumo(numeric, date, character varying, date, date)
  OWNER TO postgres;
