-- Function: eg_simulador_liquidacion_fenalco_fecha(numeric, integer, date, character varying, character varying, date)

-- DROP FUNCTION eg_simulador_liquidacion_fenalco_fecha(numeric, integer, date, character varying, character varying, date);

CREATE OR REPLACE FUNCTION eg_simulador_liquidacion_fenalco_fecha(valornegocio numeric, numcuotas integer, fechaitem date, idconvenio character varying, afiliado character varying, _fechabase date)
...
DECLARE
	...
	fechaAnterior DATE := _fechaBase::date;
...
----------------------------------------------------------------------------------------
	...
	--VALIDACION PARA LIBRANZA
        IF(idconvenio::integer=38)THEN
           vlrSeguro:=(SELECT prima FROM administrativo.temporary_seguro
			WHERE valornegocio BETWEEN cobertura_rango_ini and cobertura_rango_fin );
	ELSE
...
ALTER FUNCTION eg_simulador_liquidacion_fenalco_fecha(numeric, integer, date, character varying, character varying, date)
  OWNER TO postgres;
