-- Function: dv_ver_extracto_fenalco_ciclo_pagos(numeric, character varying, numeric, date, character varying, character varying)

-- DROP FUNCTION dv_ver_extracto_fenalco_ciclo_pagos(numeric, character varying, numeric, date, character varying, character varying);

CREATE OR REPLACE FUNCTION dv_ver_extracto_fenalco_ciclo_pagos(und_negocio numeric, periodo_corriente character varying, nciclo numeric, fecha_corte date, accion character varying, usuario character varying)
...
BEGIN
	...
	SQL = 'select
		...
		WHERE  f.reg_status = ''''
			...
			and f.negasoc in (''FA34145'') --(''FA18901'',''FA18900'')
	...
	FOR CarteraxCliente IN EXECUTE SQL  LOOP
		...
		LOOP
			...
			SELECT INTO liquidadorNegocio * from documentos_neg_aceptado where cod_neg = CarteraxCliente.negasoc;

	--	IF(CarteraxCabeceraCta.valor_saldo < liquidadorNegocio.cuota_manejo) THEN
			if ( substring(CarteraxCabeceraCta.documento,8,2) != '00' ) then

				if (liquidadorNegocio.causar_cuota_admin = 'S') then

					_CuotaManejo = sp_tienecuotamanejo_vencidas(periodo_corriente, CarteraxCabeceraCta.negocio, CarteraxCabeceraCta.cuota, Ciclo.id);
					--raise notice '_CuotaManejo: %', _CuotaManejo;
				end if;

				if (liquidadorNegocio.causar_cuota_admin = 'N') then
			...
			LOOP
				...
				*/
				...
				if ( _Base > 0 ) then
					...
					raise notice 'PREFIJO: % | DIAS VENCIDOS: % | UND NEG: %',CarteraxCuota.prefijo, CarteraxCuota.dias_vencidos, und_negocio;
					if found then
						...
						--raise notice '**_SumCapital: %',_SumCapital;
...
ALTER FUNCTION dv_ver_extracto_fenalco_ciclo_pagos(numeric, character varying, numeric, date, character varying, character varying)
  OWNER TO postgres;
