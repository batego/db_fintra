-- Function: sp_carteramicro_gacixm(character varying)

-- DROP FUNCTION sp_carteramicro_gacixm(character varying);

CREATE OR REPLACE FUNCTION sp_carteramicro_gacixm(cod_neg character varying)
...
BEGIN
   ...
   FOR listaFacturas IN EXECUTE '
			...
			WHERE fac.negasoc in '||cod_neg||'
			--AND fac.documento LIKE ''MC%''
			AND substring(fac.documento,1,2) in (''MC'',''MI'')
			AND valor_saldo > 0
			AND fac.reg_status !=''A''
			AND replace(substring(fac.fecha_vencimiento,1,7),''-'','''') <= replace(substring(now()::date,1,7),''-'','''')
			AND facdet.descripcion in (''SEGURO'',''INTERESES MICRO CORDOBA'')
	...
	LOOP
		...
		IF (listaFacturas.dias_mora > 0) THEN
			...
			**********************************/
			...
			raise notice 'tasaIg : % listaFacturas.dias_mora :% ',tasaIg,listaFacturas.dias_mora;
				IF (FOUND) THEN
		...
		ELSE
			...
			IF((resta between 29  and 34) or resta is null)THEN
                                saldoMI=filaLiquidador.interes;
				listaFacturas.valor_saldo_mi= saldoMI;
				listaFacturas.valor_saldo_ca=filaLiquidador.cat;
				saldoSeguro:=listaFacturas.valor_seguro;
			END IF;
		      ...
		      sumaSaldo= listaFacturas.valor_saldo_capital + saldoMI + filaLiquidador.cat +saldoSeguro;
                      listaFacturas.suma_saldos=sumaSaldo;
		END IF;
...
ALTER FUNCTION sp_carteramicro_gacixm(character varying)
  OWNER TO postgres;
