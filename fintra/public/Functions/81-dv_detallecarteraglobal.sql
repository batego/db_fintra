-- Function: dv_detallecarteraglobal(numeric, character varying)

-- DROP FUNCTION dv_detallecarteraglobal(numeric, character varying);

CREATE OR REPLACE FUNCTION dv_detallecarteraglobal(periodoasignacion numeric, unidadnegocio character varying)
...
$BODY$
	...
	BEGIN
		...
		FOR CarteraGeneral IN
			...
			and num_doc_fen not like '%-%'
			and num_doc_fen not like '%A%'
		...
		--NEGOCIO DE SEGURO
		---------------------------------------------------------------------------------------------------
		FOR NegocioSegurus IN SELECT cod_neg from negocios where negocio_rel_seguro = CarteraGeneral.negocio LOOP
		...
		--NEGOCIO DE GPS
		---------------------------------------------------------------------------------------------------
		SELECT INTO _NegocioGps cod_neg from negocios where negocio_rel_gps = CarteraGeneral.negocio;
...
ALTER FUNCTION dv_detallecarteraglobal(numeric, character varying)
  OWNER TO postgres;
