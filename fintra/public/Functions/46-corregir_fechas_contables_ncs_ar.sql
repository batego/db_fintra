-- Function: corregir_fechas_contables_ncs_ar()

-- DROP FUNCTION corregir_fechas_contables_ncs_ar();

CREATE OR REPLACE FUNCTION corregir_fechas_contables_ncs_ar()
  RETURNS text AS
$BODY$DECLARE
    _group RECORD;
    _count INTEGER;
...
SELECT * FROM (
	...
	FROM (
		SELECT (SELECT MAX(f_facturado_cliente)
					FROM app_accord
					WHERE factura_retencion=c.documento_relacionado
		...
		WHERE SUBSTR(documento,1,2) IN ('AR') AND proveedor='9002335631' AND tipo_documento IN ('NC')  AND numdoc=documento AND tipodoc='NC'
...
ALTER FUNCTION corregir_fechas_contables_ncs_ar()
  OWNER TO postgres;
