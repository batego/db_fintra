-- Function: get_vlrml_impfactura(text, text, text, text, text, text, text)

-- DROP FUNCTION get_vlrml_impfactura(text, text, text, text, text, text, text);

CREATE OR REPLACE FUNCTION get_vlrml_impfactura(text, text, text, text, text, text, text)
  RETURNS moneda AS
$BODY$DECLARE
...
--         Modificado Por: Ing. AndrÃƒÆ’Ã‚Â©s Maturana D.        *
--************************************************************************************
     ...
     -- Formamos fecha vigencia a partir fecha del documento.
      ...
      IF ( nParametro='IVA' OR  nParametro='RIVA'  OR  nParametro='RICA'  OR  nParametro='RFTE'  ) THEN
          ...
          -- Buscamos  el total de un tipo de Impuesto aplicado a la factura
                  SELECT
                          INTO  valor  coalesce(SUM( a.vlr_total_impuesto),0 ) as total 
                  FROM    
                          fin.cxp_imp_item  a,
                          tipo_de_impuesto  b
                  WHERE
      ...
      ELSIF  ( nParametro='BRUTO' ) THEN
              ...
              SELECT
                       INTO  valor  coalesce(SUM( a.vlr),0 )
...
ALTER FUNCTION get_vlrml_impfactura(text, text, text, text, text, text, text)
  OWNER TO postgres;
COMMENT ON FUNCTION get_vlrml_impfactura(text, text, text, text, text, text, text) IS 'Retorna valores totales de Impuesto y bruto de una factura';
