-- Function: sp_ver_extracto_fenalco(numeric, date, character varying, date, character varying, character varying)

-- DROP FUNCTION sp_ver_extracto_fenalco(numeric, date, character varying, date, character varying, character varying);

CREATE OR REPLACE FUNCTION sp_ver_extracto_fenalco(und_negocio numeric, fecha_hoy date, periodo_corriente character varying, vcto_rop date, accion character varying, usuario character varying)
...
BEGIN
	...
	DELETE FROM tem.extracto_temp;
	   ...
	   SQL = 'select
		'''||vcto_rop||'''::date as vencimiento_rop,
		''''::varchar as venc_mayor,
		id_convenio::numeric, negasoc::varchar, f.nit::varchar,
		''''::varchar as nom_cli,
		...
		('''||fecha_hoy||'''-min(fecha_vencimiento)::DATE)::numeric as min_dias_ven,
		...
		WHERE f.reg_status = ''''
			...
			--and f.negasoc = ''NG18097''
			and substring(f.documento,1,2) not in (''CP'',''FF'') ';


		IF(accion = 'Generar') then
			SQL = SQL || ' and f.negasoc in (select cod_neg from extractos_generados where periodo =' || periodo_corriente || ' and unidad_negocio =' ||  und_negocio || ' and generado =''N'')
			GROUP BY id_convenio, negasoc,f.nit ';
		ELSE
			SQL = SQL || ' and f.negasoc not in (select cod_neg from extractos_generados where unidad_negocio = ' ||  und_negocio || ' and periodo = ' || periodo_corriente || ')
			GROUP BY id_convenio, negasoc,f.nit ';
		END IF;
	...
	FOR CarteraxCliente IN EXECUTE SQL
		...
		LOOP
			...
			SELECT INTO vencimiento_mayor 	CASE WHEN maxdia >= 365 THEN '8- MAYOR A 1 ANIO'
				...
				ELSE '0' END AS rango
			FROM (
			 SELECT max(fecha_hoy::date-(fecha_vencimiento)) as maxdia
			...
			SELECT INTO _TotalCuotasVencidas count(0) as CtasVencidas from con.foto_cartera where reg_status = '' and dstrct = 'FINV' and tipo_documento in ('FAC','NDC') and negasoc = CarteraxCliente.negasoc and id_convenio = CarteraxCliente.id_convenio and valor_saldo > 0 and periodo_lote = periodo_corriente and replace(substring(fecha_vencimiento,1,7),'-','')::numeric <= periodo_corriente and substring(documento,1,2) not in ('CP','FF','DF') and descripcion not in ('CXC AVAL','CXC_INTERES_FA','CXC_CAT_MC','CXC_INTERES_MC');
			SELECT INTO _TotalCuotasFaltantes count(0) as CtasFaltantes from con.foto_cartera where reg_status = '' and dstrct = 'FINV' and tipo_documento in ('FAC','NDC') and negasoc = CarteraxCliente.negasoc  and id_convenio = CarteraxCliente.id_convenio and valor_saldo > 0 and periodo_lote = periodo_corriente and substring(documento,1,2) not in ('CP','FF','DF') and descripcion not in ('CXC AVAL','CXC_INTERES_FA','CXC_CAT_MC','CXC_INTERES_MC');
			...
			if(accion = 'Generar') then
				...
				INSERT INTO recibo_oficial_pago (cod_rop, id_unidad_negocio, periodo_rop, vencimiento_rop, negocio, cuotas_vencidas, cuotas_pendientes,
					...
					VALUES('',und_negocio,periodo_corriente,vcto_rop,CarteraxCliente.negasoc,'0','0/0','0',0,0,0,0,0,now(),usuario,now(),usuario,'')
					...
					IF NegocioAvales.cod_neg != '' THEN
						...
						FOR CarteraxCabeceraCtaAval IN
							...
							WHERE   f.reg_status = ''
								...
								and f.tipo_documento in ('FAC','NDC')
								and f.valor_saldo > 0
								--and f.id_convenio in (26,16,17,19,18)
			...
			if ( _TotalCuotasPendientes.maxdia <= 0 ) then
				...
				MsgPagueseAntes = (select max(fecha_vencimiento) from con.foto_cartera where negasoc = CarteraxCliente.negasoc and valor_saldo > 0 and periodo_lote = periodo_corriente and replace(substring(fecha_vencimiento,1,7),'-','')::numeric = periodo_corriente);
					if ( MsgPagueseAntes is null ) then
						MsgPagueseAntes = (select max(fecha_vencimiento) from con.foto_cartera where negasoc = CarteraxCliente.negasoc and valor_saldo > 0 and periodo_lote = periodo_corriente and replace(substring(fecha_vencimiento,1,7),'-','')::numeric = periodo_corriente+1);
					end if;
			...
			if(accion = 'Generar') then
				...
				UPDATE recibo_oficial_pago SET cod_rop = CodRop, cedula = CarteraxCliente.nit, nombre_cliente = ClienteRec.nombre, direccion = ClienteRec.direccion, ciudad = ClienteRec.codciu, fecha_ultimo_pago = FechaCalculoInteres, cuotas_vencidas = CarteraxCliente.total_cuotas_vencidas, cuotas_pendientes = _TotalCuotasFaltantes||' de '||_TotalCuotas, dias_vencidos = _TotalCuotasPendientes.maxdia,
					subtotal = ROUND(_SumCapital + _SumIntCte + _SumIntVencido + SumTotalIxM + SumTotalGac), total_sanciones = ROUND(SumTotalIxM + SumTotalGac), total_descuentos = ROUND(_SumDsctoCap + _SumDsctoSeguro + _SumDsctoInt + _SumDsctoIxM + _SumDsctoGxC), total = ROUND(_SumCapital +_SumIntCte + _SumIntVencido + SumTotalIxM + SumTotalGac - _TotalSumCxCta), total_abonos = 0, observacion = Mensaje, msg_paguese_antes = MsgPagueseAntes, msg_estado_credito = MsgEstadoCredito WHERE id = _cod_rop;
			end if;
			...
			CarteraxCliente.subtotal_det = ROUND(_SumCapital + _SumIntCte + _SumIntVencido + SumTotalIxM + SumTotalGac);
			CarteraxCliente.subtotalneto = ROUND(_SumCapitalCte + _SumCapitanVenc + _SumIntCte + _SumIntVencido + SumTotalIxM + SumTotalGac);
			...
			CarteraxCliente.total = ROUND(_SumCapital +_SumIntCte + _SumIntVencido + SumTotalIxM + SumTotalGac - _TotalSumCxCta);
			...
			if(accion = 'Generar') then
				...
				UPDATE extractos_generados SET generado = 'S' WHERE cod_neg = CarteraxCliente.negasoc and periodo = periodo_corriente and unidad_negocio =  und_negocio;
			end if;
...
ALTER FUNCTION sp_ver_extracto_fenalco(numeric, date, character varying, date, character varying, character varying)
  OWNER TO postgres;
