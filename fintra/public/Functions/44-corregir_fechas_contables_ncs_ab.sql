-- Function: corregir_fechas_contables_ncs_ab()

-- DROP FUNCTION corregir_fechas_contables_ncs_ab();

CREATE OR REPLACE FUNCTION corregir_fechas_contables_ncs_ab()
  RETURNS text AS
$BODY$DECLARE
    _group RECORD;
    _count INTEGER;
...
SELECT * FROM (
	...
	FROM (
		SELECT (SELECT MAX(f_facturado_cliente)
					FROM app_accord
					WHERE factura_bonificacion=c.documento_relacionado
					GROUP BY factura_bonificacion) AS fec_open
		...
		WHERE SUBSTR(documento,1,2) IN ('AB') AND proveedor='9002335631' AND tipo_documento IN ('NC')  AND numdoc=documento AND tipodoc='NC'

	) ali
) correccion_contable_ncs_ab
...
ALTER FUNCTION corregir_fechas_contables_ncs_ab()
  OWNER TO postgres;
