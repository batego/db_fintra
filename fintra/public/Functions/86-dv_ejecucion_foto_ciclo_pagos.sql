-- Function: dv_ejecucion_foto_ciclo_pagos(character varying)

-- DROP FUNCTION dv_ejecucion_foto_ciclo_pagos(character varying);

CREATE OR REPLACE FUNCTION dv_ejecucion_foto_ciclo_pagos(periodo_foto character varying)
  RETURNS text AS
$BODY$
DECLARE
...
BEGIN
	...
	SELECT INTO Ciclo * FROM con.ciclos_facturacion WHERE periodo = periodo_foto and num_ciclo = 1;
	...
	IF FOUND THEN
		...
		and f.negasoc in ('MC08423') and documento in ('CA77416','MI91370')
...
ALTER FUNCTION dv_ejecucion_foto_ciclo_pagos(character varying)
  OWNER TO postgres;
