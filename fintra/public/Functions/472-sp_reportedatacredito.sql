-- Function: sp_reportedatacredito(date, numeric, character varying, numeric, numeric, character varying)

-- DROP FUNCTION sp_reportedatacredito(date, numeric, character varying, numeric, numeric, character varying);

CREATE OR REPLACE FUNCTION sp_reportedatacredito(fecha_hoy date, periodo_corriente numeric, unidadnegocio character varying, rangoini numeric, rangofin numeric, accion character varying)
...
BEGIN
	...
	IF ( (Accion = 'Generar' OR Accion = 'Visualizar') and ( unidadnegocio in (16,17,20,21,22) ) ) THEN
		...
		FOR CarteraGeneral IN EXECUTE SQL LOOP
			...
			IF FOUND THEN
				CarteraGeneral.ultimo_pago = FchLastPay.fecha_consignacion::date;
				--CarteraGeneral.ultimo_pago = COALESCE(FchLastPay.fecha_consignacion::date,'0099-01-01'::date);
			--ELSE
				--CarteraGeneral.ultimo_pago = '0099-01-01';
			END IF;
...
ALTER FUNCTION sp_reportedatacredito(date, numeric, character varying, numeric, numeric, character varying)
  OWNER TO postgres;
