-- Function: eg_movimiento_tsp_paso(integer, character varying, character varying, character varying)

-- DROP FUNCTION eg_movimiento_tsp_paso(integer, character varying, character varying, character varying);

CREATE OR REPLACE FUNCTION eg_movimiento_tsp_paso(_filtro integer, _periodoi character varying, _periodof character varying, _tipoanticipo character varying)
   ...
   END IF;
    ...
    LOOP
		...
		IF(resultado.tipodoc IN ('EGR'))THEN
			...
			ELSE
				raise notice '1-resultado.documento_rel : %', resultado.documento_rel;
				--buscamos el documento relacionado de la cuenta de banco
					...
					raise notice '2-strpos(resultado.documento_rel) : %', strpos(resultado.documento_rel, '_');
					...
					raise notice '3-resultado.referencia_1 : %',resultado.referencia_1;

					IF(_concept_code='50' AND SUBSTRING(resultado.numdoc,1,2) ='TR' )THEN

						SELECT INTO _documentoRelacionado tipo_operacion,concept_code FROM fin.anticipos_pagos_terceros
						   WHERE planilla=resultado.referencia_1 AND reg_status='' AND  concept_code=_concept_code  and factura_mims=resultado.numdoc;
						...
						raise notice '4- _documentoRelacionado : %',_documentoRelacionado;
		...
		IF(resultado.tipodoc IN ('FAC'))THEN
			...
			IF(resultado.cuenta IN ('13802701','13802701','13802702','13802704','13802602') AND resultado.numdoc not ilike 'R0%')THEN
			...
			IF(resultado.tipodoc_rel IN (_tipoanticipo))THEN
				RAISE NOTICE 'resultado.tipodoc_rel : % _tipoanticipo: % ',resultado.tipodoc_rel,_tipoanticipo;
...
ALTER FUNCTION eg_movimiento_tsp_paso(integer, character varying, character varying, character varying)
  OWNER TO postgres;
