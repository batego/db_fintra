-- Function: sp_detallecarteraxsancion(numeric, character varying, character varying)

-- DROP FUNCTION sp_detallecarteraxsancion(numeric, character varying, character varying);

CREATE OR REPLACE FUNCTION sp_detallecarteraxsancion(periodoasignacion numeric, unidadnegocio character varying, negocioref character varying)
	...
	BEGIN
		...
		LOOP
			...
			ELSE
				...
				FOR _sanciofa IN (SELECT * FROM con.factura_detalle where documento = CarteraGeneral.documento order by descripcion) LOOP
...
ALTER FUNCTION sp_detallecarteraxsancion(numeric, character varying, character varying)
  OWNER TO postgres;
