-- Function: sp_cosechasdetallado2(numeric, numeric, character varying)

-- DROP FUNCTION sp_cosechasdetallado2(numeric, numeric, character varying);

CREATE OR REPLACE FUNCTION sp_cosechasdetallado2(periodocolocacionini numeric, periodocolocacionfin numeric, unidadnegocio character varying)
...
BEGIN
	...
	FOR CarteraGeneral IN
		...
		FROM (
			select
				...
				--(select sum(valor) from documentos_neg_aceptado where cod_neg = f.negasoc) as valor_asignado,
			...
			from con.factura ff, negocios nn
		...
		from (
			...
			where f.negasoc = n.cod_neg
				...
				and n.cod_neg not in (
						select n.cod_neg
						from negocios n
						inner join con.factura as f on (f.negasoc = n.cod_neg and f.dstrct = 'FINV' and f.tipo_documento in ('FAC','NDC') and substring(f.documento,1,2) not in ('CP','FF','DF'))
						where n.dist = 'FINV'
						and n.tneg = '03'
						and n.periodo between PeriodoColocacionIni and PeriodoColocacionFin
						and n.id_convenio in (select id_convenio from rel_unidadnegocio_convenios where id_unid_negocio = UnidadNegocio)
	...
	LOOP
		...
		--NEGOCIO
		SELECT INTO RecNegocios cod_neg, cod_cli, id_convenio, fecha_ap::date, f_desem::date, vr_desembolso, update_user
		...
		CarteraGeneral.total_desembolsado = RecNegocios.vr_desembolso;
		...
		SELECT INTO misDias
			CASE WHEN maxdia >= 361 THEN '8- MAYOR A 1 AÑO'
				...
				ELSE '0' END AS rango
		FROM (
			SELECT MAX((fecha_ultimo_pago-fecha_vencimiento)) as maxdia
			FROM con.factura fra
			WHERE fra.dstrct = 'FINV'
			      AND fra.reg_status = ''
			      AND fra.negasoc = CarteraGeneral.negocio
			      AND replace(substring(fecha_vencimiento,1,7),'-','')::numeric < PeriodoTramo
		...
		LOOP
			...
			--CarteraGeneral.asesor_comercial = ;
...
ALTER FUNCTION sp_cosechasdetallado2(numeric, numeric, character varying)
  OWNER TO postgres;
