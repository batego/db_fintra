-- Function: eg_movimiento_tsp_paso5(integer, character varying, character varying, character varying)

-- DROP FUNCTION eg_movimiento_tsp_paso5(integer, character varying, character varying, character varying);

CREATE OR REPLACE FUNCTION eg_movimiento_tsp_paso5(_filtro integer, _periodoi character varying, _periodof character varying, _tipoanticipo character varying)
...
_arrCuentasATE varchar[] := '{13802702}';
_arrCuentasAGA varchar[] := '{23050118}';
_arrCuentasEXT varchar[] := '{1,1}';
...
BEGIN
	...
	IF(_tipoanticipo='AET')THEN
		filtro2:=' AND a.cuenta in ('''||_arrCuentasATE[1]||''')';
		_concept_code:='01';
	ELSIF(_tipoanticipo='AGA') THEN
		filtro2:=' AND  a.cuenta in ('''||_arrCuentasAGA[1]||''')';
	...
	LOOP
		...
		IF(resultado.tipodoc='FAC')THEN
		   resultado.tipodoc_rel:=resultado.tipodoc;
		   resultado.documento_rel:=resultado.numdoc;
		END IF;
...
ALTER FUNCTION eg_movimiento_tsp_paso5(integer, character varying, character varying, character varying)
  OWNER TO postgres;
