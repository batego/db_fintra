-- Function: eg_preaprobados_creditos(integer)

-- DROP FUNCTION eg_preaprobados_creditos(integer);

CREATE OR REPLACE FUNCTION eg_preaprobados_creditos(_unidadnegocio integer)
  RETURNS SETOF rs_preaprobado AS
$BODY$
			...
			IF(recordNegocios.altura_mora_actual = '1- CORRIENTE' )	THEN
				...
				if(recordNegocios.porcetaje = 100.00 and recordNegocios.dias_ultimmo_pago <=180 )then
					...
					if(recordNegocios.altura_mora::integer <= 30)then
						...
						recordNegocios.valor_preaprobado:= eg_valor_preaprobado(recordNegocios.vr_negocio,_unidadNegocio,'PREAPROBADO');
...
ALTER FUNCTION eg_preaprobados_creditos(integer)
  OWNER TO postgres;
