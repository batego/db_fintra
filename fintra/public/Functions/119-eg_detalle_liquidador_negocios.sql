-- Function: eg_detalle_liquidador_negocios(integer)

-- DROP FUNCTION eg_detalle_liquidador_negocios(integer);

CREATE OR REPLACE FUNCTION eg_detalle_liquidador_negocios(_unidadnegocio integer)
...
BEGIN
	...
	FOR record_liquidador IN (SELECT
					...
					,case when ends.estado::varchar is null then 'NO ENDOSADO'
					      else ends.estado::varchar
						 end as estado_endoso
				FROM negocios neg
				INNER JOIN documentos_neg_aceptado dna on (dna.cod_neg=neg.cod_neg)
				LEFT JOIN ing_fenalco ing ON (dna.cod_neg=ing.codneg and ing.cuota=dna.item::integer AND ing.tipodoc not in ('CM','CA'))
				...
				LEFT JOIN negocios_endosados ends on (ends.negocio=neg.cod_neg)
				...
				AND dna.reg_status=''
				--AND dna.cod_neg in ('MC10677','MC09797','MC11073','MC10914')
				and neg.id_convenio in (select id_convenio from rel_unidadnegocio_convenios where id_unid_negocio in (select id from unidad_negocio where id = _unidadNegocio))
...
ALTER FUNCTION eg_detalle_liquidador_negocios(integer)
  OWNER TO postgres;
