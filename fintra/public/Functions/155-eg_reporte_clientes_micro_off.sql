-- Function: eg_reporte_clientes_micro_off(numeric, date, character varying)

-- DROP FUNCTION eg_reporte_clientes_micro_off(numeric, date, character varying);

CREATE OR REPLACE FUNCTION eg_reporte_clientes_micro_off(peridofoto numeric, fechacorte date, unidadnegocio character varying)
  RETURNS SETOF record AS
$BODY$
DECLARE

	recordClientes record;
	valorSaldo numeric :=0;
...
BEGIN
	...
	FOR recordClientes IN
		...
		SELECT
			...
			0::numeric as valor_saldo_fac,
			'0099-01-01'::date as fecha_ulti_pago,
	...
	LOOP
		...
		SELECT INTO valorSaldo sum(valor_saldo) as valor_saldo
		FROM con.factura
		WHERE negasoc=recordClientes.negasoc
		AND reg_status='' ;

		recordClientes.valor_saldo_fac:=valorSaldo;

		SELECT INTO fecha_pago_ultimo max(fecha_ultimo_pago) as fecha_ultimo_pago  FROM con.factura
		WHERE negasoc=recordClientes.negasoc AND reg_status='';

		recordClientes.fecha_ulti_pago:=fecha_pago_ultimo;
...
ALTER FUNCTION eg_reporte_clientes_micro_off(numeric, date, character varying)
  OWNER TO postgres;
