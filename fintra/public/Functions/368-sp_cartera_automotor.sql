-- Function: sp_cartera_automotor(character varying, character varying, character varying, character varying)

-- DROP FUNCTION sp_cartera_automotor(character varying, character varying, character varying, character varying);

CREATE OR REPLACE FUNCTION sp_cartera_automotor(periodo_foto character varying, fch_corte character varying, fch_corte_periodo_anterior character varying, periodo_recaudo character varying)
	...
	FOR CarteraxCliente IN
		...
		FROM (
			...
			where --f.negasoc = ng.cod_neg
				...
				--and f.negasoc in ('FA03546','FA03752') --MC00632 'MC03652'--in ('MC03652','MC00208','MC00494')
			order by negasoc)
			...
			where --f.negasoc = ng.cod_neg
				...
				and substring(f.documento,1,2) not in ('CP','FF','DF') --,'CC'
				and f.periodo_lote = periodo_foto
				and f.negasoc in (select negocio_seguro from tem.seguros_vehiculos where ciclo_fecha = '2014-05-14')
			order by negasoc)
		...
		FOR BankPay IN
			...
			select i.num_ingreso, id.factura, i.branch_code, i.bank_account_no, i.fecha_consignacion, sum(id.valor_ingreso) as mingreso
			...
			group by i.num_ingreso, id.factura, i.branch_code, i.bank_account_no, i.fecha_consignacion LOOP
...
ALTER FUNCTION sp_cartera_automotor(character varying, character varying, character varying, character varying)
  OWNER TO postgres;
