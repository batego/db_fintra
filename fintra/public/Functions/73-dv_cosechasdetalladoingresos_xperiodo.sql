-- Function: dv_cosechasdetalladoingresos_xperiodo(character varying)

-- DROP FUNCTION dv_cosechasdetalladoingresos_xperiodo(character varying);

CREATE OR REPLACE FUNCTION dv_cosechasdetalladoingresos_xperiodo(unidadnegocio character varying)
...
BEGIN
   ...
   -- raise notice 'PeriodoColocacionIni: % PeriodoColocacionFin: %',PeriodoColocacionIni,PeriodoColocacionFin;
    ...
    FOR CarteraGeneral IN
        ...
        FROM (
            ...
            where ff.negasoc = nn.cod_neg
                ...
                and nn.periodo between _TramoAnterior and PeriodoTramo
        ...
        (select salario from solicitud_laboral where numero_solicitud = (select numero_solicitud from solicitud_aval where cod_neg = c.negocio) and tipo ='S') as ingresos,
        c.periodo
        from (
            ...
            where f.negasoc = n.cod_neg
                ...
                and n.periodo between _TramoAnterior and PeriodoTramo
            ...
            group by negasoc, num_doc_fen,n.periodo
            order by negasoc,n.periodo
        ) c
        group by negocio,c.periodo
        ...
        from (
            ...
            '0-CANCELACION TOTAL'::varchar as vencimiento_mayor,
            ...
            group by negocio, vencimiento_mayor,n.periodo
            having sum(f.valor_factura) = sum(f.valor_abono)
        ) c
         group by negocio,c.vencimiento_mayor,c.periodo,c.colocacion,c.pagos
        ...
        from (
          ...
          group by negocio,c.periodo,c.vencimiento_mayor,c.colocacion,c.pagos
    LOOP
	...
	--VALIDAMOS LA CARTERA INDEMNIZADA.
	CarteraGeneral.vencimiento_mayor:=COALESCE((SELECT CASE WHEN count(0)>0 THEN '9-INDEMNIZADO GARANTIAS' END   FROM administrativo.control_indemnizacion_fianza where negocio =CarteraGeneral.negocio),CarteraGeneral.vencimiento_mayor);
...
ALTER FUNCTION dv_cosechasdetalladoingresos_xperiodo(character varying)
  OWNER TO postgres;
