-- Function: corregir_fechas_contables_cxps_ab()

-- DROP FUNCTION corregir_fechas_contables_cxps_ab();

CREATE OR REPLACE FUNCTION corregir_fechas_contables_cxps_ab()
  RETURNS text AS
$BODY$DECLARE
    _group RECORD;
    _count INTEGER;
...
SELECT * FROM (
	...
	FROM (
		SELECT (SELECT MAX(f_facturado_cliente)
					FROM app_accord
					WHERE factura_bonificacion=c.documento
		...
		WHERE SUBSTR(documento,1,2) IN ('AB') AND proveedor='9002335631' AND tipo_documento IN ('FAP')  AND numdoc=documento AND tipodoc='FAP'

	) ali
) correccion_contable_cxps_ab
...
ALTER FUNCTION corregir_fechas_contables_cxps_ab()
  OWNER TO postgres;
