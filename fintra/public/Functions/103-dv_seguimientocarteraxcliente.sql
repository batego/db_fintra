-- Function: dv_seguimientocarteraxcliente(numeric, character varying, character varying, character varying)

-- DROP FUNCTION dv_seguimientocarteraxcliente(numeric, character varying, character varying, character varying);

CREATE OR REPLACE FUNCTION dv_seguimientocarteraxcliente(periodoasignacion numeric, unidadnegocio character varying, agenteext character varying, nitcliente character varying)
...
BEGIN
	...
	FOR CarteraGeneral IN
		...
		where periodo_lote = PeriodoAsignacion
			...
			--and (SELECT count(0) from negocios where cod_neg = con.foto_cartera.negasoc and negocio_rel = '') > 0 --//Se comenta para que sume el negocio aval
...
ALTER FUNCTION dv_seguimientocarteraxcliente(numeric, character varying, character varying, character varying)
  OWNER TO postgres;
