-- Function: corregir_fechas_contables_cxps_contratista()

-- DROP FUNCTION corregir_fechas_contables_cxps_contratista();

CREATE OR REPLACE FUNCTION corregir_fechas_contables_cxps_contratista()
  RETURNS text AS
$BODY$DECLARE
    _group RECORD;
    _count INTEGER;
...
SELECT * FROM (
	...
	FROM (
		SELECT (SELECT MAX(f_facturado_cliente)
					FROM app_accord
					WHERE factura_contratista=c.documento
		...
		WHERE SUBSTR(documento,1,3) IN ('CC0') AND tipo_documento IN ('FAP')  AND numdoc=documento AND tipodoc='FAP'
	) ali
) correccion_contable_cxps_contratistas
...
ALTER FUNCTION corregir_fechas_contables_cxps_contratista()
  OWNER TO postgres;
