-- Function: eg_saldos_reestructuracion_fenalco(character varying, character varying)

-- DROP FUNCTION eg_saldos_reestructuracion_fenalco(character varying, character varying);

CREATE OR REPLACE FUNCTION eg_saldos_reestructuracion_fenalco(codigo_neg character varying, distrito character varying)
...
BEGIN
   ...
   FOR listaFacturas IN (
			...
			AND facdet.reg_status !='A'
				...
				ELSE facdet.descripcion in ('INTERESES','AVAL','PG','CH','BQ-015-FENALCO-CUOTA DE MANEJO') --Agrego concepto cuota de manejo
				END
...
ALTER FUNCTION eg_saldos_reestructuracion_fenalco(character varying, character varying)
  OWNER TO postgres;
