-- Function: sp_cartera_educativo(character varying, character varying, character varying, character varying)

-- DROP FUNCTION sp_cartera_educativo(character varying, character varying, character varying, character varying);

CREATE OR REPLACE FUNCTION sp_cartera_educativo(periodo_foto character varying, fch_corte character varying, fch_corte_periodo_anterior character varying, periodo_recaudo character varying)
...
BEGIN
	...
	FOR CarteraxCliente IN
		...
		FROM (
			...
			f.id_convenio as convenio,
			f.nit AS cedula,
			fecha_negocio as fecha_desembolso,
			replace(substring(fecha_negocio,1,7),'-','') as periodo_desembolso,
			f.documento,
			...
			where f.id_convenio in (17,19,31)
				and f.valor_saldo > 0
				and f.reg_status = ''
				and f.dstrct = 'FINV'
...
ALTER FUNCTION sp_cartera_educativo(character varying, character varying, character varying, character varying)
  OWNER TO postgres;
