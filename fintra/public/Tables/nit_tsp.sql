...
(
  ...
  coddpto character varying(10) NOT NULL DEFAULT ''::character varying,
  ...
  usuariocrea character varying(15) NOT NULL DEFAULT ''::character varying,
  ...
  nombre1 character varying(15) NOT NULL DEFAULT ''::character varying,
  nombre2 character varying(15) NOT NULL DEFAULT ''::character varying,
  apellido1 character varying(30) NOT NULL DEFAULT ''::character varying,
  apellido2 character varying(15) NOT NULL DEFAULT ''::character varying,
  ...
  barrio character varying(25) NOT NULL DEFAULT ''::character varying,
  ...
  cupo_carbon numeric(10,3) NOT NULL DEFAULT 0,
  ...
  CONSTRAINT "cedulaNoVacia" CHECK (ltrim(cedula::text) <> ''::text) -- Evita ingresar una cedula vacia en la tabla de nits
)
WITH (
  OIDS=TRUE
);
ALTER TABLE nit_tsp
  OWNER TO postgres;
GRANT ALL ON TABLE nit_tsp TO postgres;
GRANT SELECT ON TABLE nit_tsp TO msoto;
COMMENT ON COLUMN nit_tsp.sexo IS 'Genero del nit';
COMMENT ON COLUMN nit_tsp.fechanac IS 'Fecha de Nacimiento';
COMMENT ON COLUMN nit_tsp.veto IS 'Id. de vetos';
COMMENT ON COLUMN nit_tsp.aprobado IS 'Indica si el registro esta aprobado o no.';
COMMENT ON COLUMN nit_tsp.usuario_aprobacion IS 'Registra el usuario que aprobo el registro.';
COMMENT ON COLUMN nit_tsp.fecha_aprobacion IS 'Registra la fecha en que se aprobo el registro.';
COMMENT ON COLUMN nit_tsp.pk_novedad IS 'llave primaria';
COMMENT ON COLUMN nit_tsp.fecha_envio_ws IS 'fecha en la que se envio por ultima vez el registro del servidor al cliente  a traves del web service';
COMMENT ON COLUMN nit_tsp.fecha_anulacion IS 'fecha en la que se anula el registro';
COMMENT ON COLUMN nit_tsp.creation_date_real IS 'fecha de creacion real';

COMMENT ON CONSTRAINT "cedulaNoVacia" ON nit_tsp IS 'Evita ingresar una cedula vacia en la tabla de nits';


-- Trigger: nittspafinvt on nit_tsp

-- DROP TRIGGER nittspafinvt ON nit_tsp;

CREATE TRIGGER nittspafinvt
  AFTER INSERT OR UPDATE
  ON nit_tsp
  FOR EACH ROW
  EXECUTE PROCEDURE nittspafinv();
