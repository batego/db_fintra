-- Table: ex_subclientes_ofertas

-- DROP TABLE ex_subclientes_ofertas;

CREATE TABLE ex_subclientes_ofertas
(
  ...
  clase_dtf character varying(10) NOT NULL DEFAULT ''::character varying,
...
ALTER TABLE ex_subclientes_ofertas
  OWNER TO postgres;
GRANT ALL ON TABLE ex_subclientes_ofertas TO postgres;
GRANT SELECT ON TABLE ex_subclientes_ofertas TO msoto;
COMMENT ON COLUMN ex_subclientes_ofertas.val_cuota_inicial IS 'indispensable';
COMMENT ON COLUMN ex_subclientes_ofertas.periodo IS 'cantidad de cuotas';
COMMENT ON COLUMN ex_subclientes_ofertas.valor_base IS 'valor establecido incialmente como el que va a ser financiado, a menos que haya cuota inicial y/o anticipos';
COMMENT ON COLUMN ex_subclientes_ofertas.estudio_economico IS 'parece ser una constante que determina algo asociado a la financiacion pero todavia no se sabe la trascendencia de este campo ';
COMMENT ON COLUMN ex_subclientes_ofertas.fecha_factura IS 'temporalmente la fecha en que se subiÃ³n en open';
COMMENT ON COLUMN ex_subclientes_ofertas.fecha_inicio_pago IS 'Fecha en la cual el cliente paga su primera factura. Se calculan intereses adicionales desde la fecha de fecha de factura hasta esta fecha';
