...
-- DROP TABLE conveniosx;

CREATE TABLE conveniosx
(
  ...
  popo character varying(15) NOT NULL DEFAULT ''::character varying
)
WITH (
  OIDS=FALSE
);
ALTER TABLE conveniosx
  OWNER TO postgres;
GRANT ALL ON TABLE conveniosx TO postgres;
GRANT SELECT ON TABLE conveniosx TO msoto;
COMMENT ON TABLE conveniosx
  IS 'Parametrización de convenidos y sus condiciones';
COMMENT ON COLUMN conveniosx.porcentaje_gmf IS 'Porcentaje para calcular el gravamen al movimiento financiero';
COMMENT ON COLUMN conveniosx.impuesto IS 'codigo del impuesto a aplicar';
COMMENT ON COLUMN conveniosx.cuenta_ajuste IS 'cuenta para realizar el ahuste al peso';


-- Trigger: tu_prov_convenio on conveniosx

-- DROP TRIGGER tu_prov_convenio ON conveniosx;
...
COMMENT ON TRIGGER tu_prov_convenio ON conveniosx IS 'Actualiza la tasa y/o la custodia de la asignacion de convenios de los afiliados tomando los nuevos  valores definidos en el convenio  ';
